...
describe('ReviewBillableWeight', () => {
  ...
  describe('check that all the components render', () => {
    ...
    it('renders max billable weight and edit view', async () => {
      ...
      await waitFor(() => {
        expect(screen.getByTestId('maxWeight-weightAllowance')).toHaveTextContent(weightAllowance);
        expect(screen.getByTestId('maxWeight-estimatedWeight')).toHaveTextContent('11,000 lbs');
      });
    });
  });
  ...
  describe('check that the various alerts show up when expected', () => {
    it('renders max billable weight alert in shipment view when billable weight is exceeded', async () => {
      useOrdersDocumentQueries.mockReturnValue(useOrdersDocumentQueriesReturnValue);
      useMovePaymentRequestsQueries.mockReturnValue(useMovePaymentRequestsReturnValue);
    ...
    it('does not render a max billable weight alert in shipment view when billable weight is not exceeded', async () => {
      useOrdersDocumentQueries.mockReturnValue(useOrdersDocumentQueriesReturnValue);
      useMovePaymentRequestsQueries.mockReturnValue(useNonMaxBillableWeightExceededReturnValue);
