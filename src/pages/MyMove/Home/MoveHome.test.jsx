...
describe('Home component', () => {
  ...
  const defaultPropsWithEditedAdvanceAndPPMNeedsCloseout = {
    ...props,
    serviceMemberMoves: {
      currentMove: [
        {
  ...
  const renderMoveHomeWithProviders = (defaultProps) => {
    const moveId = defaultProps.serviceMemberMoves.currentMove[0].id;
    return mount(
      <MockProviders path={customerRoutes.MOVE_HOME_PATH} params={{ moveId }}>
        <MoveHome {...defaultProps} />
  ...
  describe('with default props, and completed ppm', () => {
    it('shows the correct ppm submission confirmation', async () => {
      // const wrapper = mountMoveHomeWithProviders(defaultPropsWithEditedAdvanceAndPPMApproved);
      renderMoveHomeWithProviders(defaultPropsWithEditedAdvanceAndPPMNeedsCloseout);
      screen.debug();
