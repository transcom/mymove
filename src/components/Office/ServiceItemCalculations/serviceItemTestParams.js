const EscalationCompounded = {
  eTag: 'MjAyMS0wMy0xOFQwMTozMTo1MS4yNzc2MTda',
  id: '6c7f1673-1ada-44fe-aa9b-e921d6e15f0e',
  key: 'EscalationCompounded',
  origin: 'PRICER',
  paymentServiceItemID: '28039a62-387d-479f-b50f-e0041b7e6e22',
  type: 'DECIMAL',
  value: '1.033',
};
const IsPeak = {
  eTag: 'MjAyMS0wMy0xOFQwMTozMTo1MS4yODc3NDla',
  id: '83f24c0d-25ab-465a-b60b-d27bfb77b41a',
  key: 'IsPeak',
  origin: 'PRICER',
  paymentServiceItemID: '28039a62-387d-479f-b50f-e0041b7e6e22',
  type: 'BOOLEAN',
  value: 'FALSE',
};
const PriceRateOrFactor = {
  eTag: 'MjAyMS0wMy0xOFQwMTozMTo1MS4yOTc2ODJa',
  id: 'b3ca0c12-fea3-4dd1-b228-30c1cc007452',
  key: 'PriceRateOrFactor',
  origin: 'PRICER',
  paymentServiceItemID: '28039a62-387d-479f-b50f-e0041b7e6e22',
  type: 'DECIMAL',
  value: '1.71',
};
const ActualPickupDate = {
  eTag: 'MjAyMS0wMy0xOFQwMTozMTo1MS4zMDY2Nzha',
  id: '0e908b35-e61b-47c5-b4bc-f1649aa1cdc2',
  key: 'ActualPickupDate',
  origin: 'PRIME',
  paymentServiceItemID: '28039a62-387d-479f-b50f-e0041b7e6e22',
  type: 'DATE',
  value: '2020-03-11',
};
const ReferenceDate = {
  eTag: 'MjAyMS0wMy0xOFQwMTozMTo1MS4zMDY2Nzha',
  id: '0e908b35-e61b-47c5-b4bc-f1649aa1cdc2',
  key: 'ReferenceDate',
  origin: 'PRIME',
  paymentServiceItemID: '28039a62-387d-479f-b50f-e0041b7e6e22',
  type: 'DATE',
  value: '2020-03-09',
};
const ServiceAreaOrigin = {
  eTag: 'MjAyMS0wMy0xOFQwMTozMTo1MS4zMTY5NDha',
  id: '87e77d29-d8c9-4b74-b45f-6842cd3ef970',
  key: 'ServiceAreaOrigin',
  origin: 'SYSTEM',
  paymentServiceItemID: '28039a62-387d-479f-b50f-e0041b7e6e22',
  type: 'STRING',
  value: '176',
};
const SITServiceAreaOrigin = {
  eTag: 'MjAyMS0wMy0xOFQwMTozMTo1MS4zMTY5NDha',
  id: '87e77d29-d8c9-4b74-b45f-6842cd3ef970',
  key: 'SITServiceAreaOrigin',
  origin: 'SYSTEM',
  paymentServiceItemID: '28039a62-387d-479f-b50f-e0041b7e6e22',
  type: 'STRING',
  value: '176',
};
const SITRateAreaOrigin = {
  eTag: 'MjAyMS0wMy0xOFQwMTozMTo1MS4zMTY5NDha',
  id: '87e77d29-d8c9-4b74-b45f-6842cd3ef970',
  key: 'SITRateAreaOrigin',
  origin: 'SYSTEM',
  paymentServiceItemID: '28039a62-387d-479f-b50f-e0041b7e6e22',
  type: 'STRING',
  value: '176',
};
const WeightAdjusted = {
  eTag: 'MjAyMS0wMy0xOFQwMTozMTo1MS4zMjY2NDVa',
  id: '70abd9bc-afaa-4e4d-ad15-d3e55b57d2fb',
  key: 'WeightAdjusted',
  origin: 'PRIME',
  paymentServiceItemID: '28039a62-387d-479f-b50f-e0041b7e6e22',
  type: 'INTEGER',
  value: '500',
};
const WeightReweigh = {
  eTag: 'MjAyMS0wMy0xOFQwMTozMTo1MS4zMjY2NDVa',
  id: '70abd9bc-afaa-4e4d-ad15-d3e55b57d2fb',
  key: 'WeightReweigh',
  origin: 'PRIME',
  paymentServiceItemID: '28039a62-387d-479f-b50f-e0041b7e6e22',
  type: 'INTEGER',
  value: '8500',
};
const WeightOriginal = {
  eTag: 'MjAyMS0wMy0xOFQwMTozMTo1MS4zMjY2NDVa',
  id: '70abd9bc-afaa-4e4d-ad15-d3e55b57d2fb',
  key: 'WeightOriginal',
  origin: 'PRIME',
  paymentServiceItemID: '28039a62-387d-479f-b50f-e0041b7e6e22',
  type: 'INTEGER',
  value: '8500',
};
const WeightBilled = {
  eTag: 'MjAyMS0wMy0xOFQwMTozMTo1MS4zMzU1Njda',
  id: '5a993802-1504-4415-9b18-fdb1fdfd201c',
  key: 'WeightBilled',
  origin: 'SYSTEM',
  paymentServiceItemID: '28039a62-387d-479f-b50f-e0041b7e6e22',
  type: 'INTEGER',
  value: '8500',
};
const WeightEstimated = {
  eTag: 'MjAyMS0wMy0xOFQwMTozMTo1MS4zNDQxMTda',
  id: '02438e39-de6c-4c64-b817-9932ee319a4c',
  key: 'WeightEstimated',
  origin: 'PRIME',
  paymentServiceItemID: '28039a62-387d-479f-b50f-e0041b7e6e22',
  type: 'INTEGER',
  value: '8000',
};
const ZipDestAddress = {
  eTag: 'MjAyMS0wMy0xOFQwMTozMTo1MS4zNTI1MDZa',
  id: 'b26fcc8f-2c06-4b00-8b51-4715a2eb0f33',
  key: 'ZipDestAddress',
  origin: 'PRIME',
  paymentServiceItemID: '28039a62-387d-479f-b50f-e0041b7e6e22',
  type: 'STRING',
  value: '91910',
};
const ZipSITDestHHGOriginalAddress = {
  eTag: 'MjAyMS0wMy0xOFQwMTozMTo1MS4zNTI1MDZa',
  id: 'b26fcc8f-2c06-4b00-8b51-4715a2eb0f33',
  key: 'ZipSITDestHHGOriginalAddress',
  origin: 'SYSTEM',
  paymentServiceItemID: '28039a62-387d-479f-b50f-e0041b7e6e22',
  type: 'STRING',
  value: '91910',
};
const ZipPickupAddress = {
  eTag: 'MjAyMS0wMy0xOFQwMTozMTo1MS4zNjA5MTha',
  id: 'dcfa55b2-3106-4e1b-af4a-f19d82b5f446',
  key: 'ZipPickupAddress',
  origin: 'PRIME',
  paymentServiceItemID: '28039a62-387d-479f-b50f-e0041b7e6e22',
  type: 'STRING',
  value: '32210',
};
const DistanceZip = {
  eTag: 'MjAyMS0wMy0xOFQwMTozMTo1MS4yNjY4M1o=',
  id: 'b4ba804d-f661-4df1-a488-11da9668647b',
  key: 'DistanceZip',
  origin: 'SYSTEM',
  paymentServiceItemID: '28039a62-387d-479f-b50f-e0041b7e6e22',
  type: 'INTEGER',
  value: '210',
};

const ContractCode = {
  eTag: 'MjAyMS0wMy0xOFQwMTozMTo1MS4yNDYwMDRa',
  id: 'f2a3e73f-6450-43d6-a783-181501cfab22',
  key: 'ContractCode',
  origin: 'SYSTEM',
  paymentServiceItemID: '28039a62-387d-479f-b50f-e0041b7e6e22',
  type: 'STRING',
  value: '1',
};

const ContractYearName = {
  eTag: 'MjAyMS0wMy0xOFQwMTozMTo1MS4yNTYzOVo=',
  id: '0ccef02a-59da-44d7-8258-f0e24c6c9b97',
  key: 'ContractYearName',
  origin: 'PRICER',
  paymentServiceItemID: '28039a62-387d-479f-b50f-e0041b7e6e22',
  type: 'STRING',
  value: '2',
};
const NumberDaysSIT = {
  eTag: 'MjAyMS0wMy0xOFQwMTozMTo1MS4yNjY4M1o=',
  id: '8c0c57e2-0acc-4319-9757-59608936a154',
  key: 'NumberDaysSIT',
  origin: 'PRIME',
  paymentServiceItemID: '28039a62-387d-479f-b50f-e0041b7e6e22',
  type: 'INTEGER',
  value: '2',
};
const CubicFeetBilled = {
  eTag: 'MjAyMS0wMy0xOFQwMTozMTo1MS4yNjY4M1o=',
  id: '1fe986ae-dbff-4fe1-b528-714560f7d2f5',
  key: 'CubicFeetBilled',
  origin: 'SYSTEM',
  paymentServiceItemID: '28039a62-387d-479f-b50f-e0041b7e6e22',
  type: 'DECIMAL',
  value: '4.00',
};
const DistanceZipSITOrigin = {
  eTag: 'MjAyMS0wMy0xOFQwMTozMTo1MS4yNjY4M1o=',
  id: 'f9753611-4b3e-4bf5-8e00-6d9ce9900f50',
  key: 'DistanceZipSITOrigin',
  origin: 'SYSTEM',
  paymentServiceItemID: '28039a62-387d-479f-b50f-e0041b7e6e22',
  type: 'INTEGER',
  value: '29',
};
const DistanceZipSITDest = {
  eTag: 'MjAyMS0wMy0xOFQwMTozMTo1MS4yNjY4M1o=',
  id: '45ede48b-364d-473a-8c61-0f520a6a4e04',
  key: 'DistanceZipSITDest',
  origin: 'SYSTEM',
  paymentServiceItemID: '28039a62-387d-479f-b50f-e0041b7e6e22',
  type: 'INTEGER',
  value: '29',
};

const DistanceZipSITDestMatchingZip3 = {
  eTag: 'MjAyMS0wMy0xOFQwMTozMTo1MS4yNjY4M1o=',
  id: '45ede48b-364d-473a-8c61-0f520a6a4e04',
  key: 'DistanceZipSITDest',
  origin: 'SYSTEM',
  paymentServiceItemID: '28039a62-387d-479f-b50f-e0041b7e6e22',
  type: 'INTEGER',
  value: '3',
};

const DistanceZipSITDestLonghaul = {
  eTag: 'MjAyMS0wMy0xOFQwMTozMTo1MS4yNjY4M1o=',
  id: '45ede48b-364d-473a-8c61-0f520a6a4e04',
  key: 'DistanceZipSITDest',
  origin: 'SYSTEM',
  paymentServiceItemID: '28039a62-387d-479f-b50f-e0041b7e6e22',
  type: 'INTEGER',
  value: '51',
};

const ZipSITOriginHHGOriginalAddress = {
  eTag: 'MjAyMS0wMy0xOFQwMTozMTo1MS4yNjY4M1o=',
  id: '70eecf7f-beae-4906-95ba-cbfe6797cf3a',
  key: 'ZipSITOriginHHGOriginalAddress',
  origin: 'PRIME',
  paymentServiceItemID: '28039a62-387d-479f-b50f-e0041b7e6e22',
  type: 'STRING',
  value: '90210',
};

const ZipSITOriginHHGActualAddress = {
  eTag: 'MjAyMS0wMy0xOFQwMTozMTo1MS4yNjY4M1o=',
  id: '70eecf7f-beae-4906-95ba-cbfe6797cf3a',
  key: 'ZipSITOriginHHGActualAddress',
  origin: 'PRIME',
  paymentServiceItemID: '28039a62-387d-479f-b50f-e0041b7e6e22',
  type: 'STRING',
  value: '90211',
};

const ZipSITDestHHGFinalAddress = {
  eTag: 'MjAyMS0wMy0xOFQwMTozMTo1MS4yNjY4M1o=',
  id: '70eecf7f-beae-4906-95ba-cbfe6797cf3a',
  key: 'ZipSITDestHHGFinalAddress',
  origin: 'PRIME',
  paymentServiceItemID: '28039a62-387d-479f-b50f-e0041b7e6e22',
  type: 'STRING',
  value: '94535',
};

const ZipSITDestHHGFinalAddressMatchingZip3 = {
  eTag: 'MjAyMS0wMy0xOFQwMTozMTo1MS4yNjY4M1o=',
  id: '70eecf7f-beae-4906-95ba-cbfe6797cf3a',
  key: 'ZipSITDestHHGFinalAddress',
  origin: 'PRIME',
  paymentServiceItemID: '28039a62-387d-479f-b50f-e0041b7e6e22',
  type: 'STRING',
  value: '91920',
};

const EIAFuelPrice = {
  eTag: 'MjAyMS0wMy0xOFQwMTozMTo1MS4yNjY4M1o=',
  id: 'adeb57e5-6b1c-4c0f-b5c9-9e57e600303f',
  key: 'EIAFuelPrice',
  origin: 'SYSTEM',
  paymentServiceItemID: '28039a62-387d-479f-b50f-e0041b7e6e22',
  type: 'INTEGER',
  value: '273300',
};
const FSCPriceDifferenceInCents = {
  eTag: 'MjAyMS0wMy0xOFQwMTozMTo1MS4yNjY4M1o=',
  id: 'adeb57e5-6b1c-4c0f-b5c9-9e57e600303f',
  key: 'FSCPriceDifferenceInCents',
  origin: 'SYSTEM',
  paymentServiceItemID: '28039a62-387d-479f-b50f-e0041b7e6e22',
  type: 'INTEGER',
  value: '200',
};
const FSCWeightBasedDistanceMultiplier = {
  eTag: 'MjAyMS0wMy0xOFQwMTozMTo1MS4yNjY4M1o=',
  id: 'adeb57e5-6b1c-4c0f-b5c9-9e57e600303f',
  key: 'FSCWeightBasedDistanceMultiplier',
  origin: 'SYSTEM',
  paymentServiceItemID: '28039a62-387d-479f-b50f-e0041b7e6e22',
  type: 'DECIMAL',
  value: '0.000417',
};
const ServiceAreaDest = {
  eTag: 'MjAyMS0wMy0xOFQwMTozMTo1MS4yNjY4M1o=',
  id: 'af92f0ca-f669-4483-95d2-d66e9c0c69e4',
  key: 'ServiceAreaDest',
  origin: 'SYSTEM',
  paymentServiceItemID: '28039a62-387d-479f-b50f-e0041b7e6e22',
  type: 'STRING',
  value: '080',
};
const SITServiceAreaDest = {
  eTag: 'MjAyMS0wMy0xOFQwMTozMTo1MS4yNjY4M1o=',
  id: 'af92f0ca-f669-4483-95d2-d66e9c0c69e4',
  key: 'ServiceAreaDest',
  origin: 'SYSTEM',
  paymentServiceItemID: '28039a62-387d-479f-b50f-e0041b7e6e22',
  type: 'STRING',
  value: '080',
};
const SITRateAreaDest = {
  eTag: 'MjAyMS0wMy0xOFQwMTozMTo1MS4yNjY4M1o=',
  id: 'af92f0ca-f669-4483-95d2-d66e9c0c69e4',
  key: 'SITRateAreaDest',
  origin: 'SYSTEM',
  paymentServiceItemID: '28039a62-387d-479f-b50f-e0041b7e6e22',
  type: 'STRING',
  value: '080',
};
const SITScheduleOrigin = {
  eTag: 'MjAyMS0wMy0xOFQwMTozMTo1MS4yNjY4M1o=',
  id: 'edeb108a-3aa8-4e7c-9571-de81951cbb51',
  key: 'SITScheduleOrigin',
  origin: 'SYSTEM',
  paymentServiceItemID: '28039a62-387d-479f-b50f-e0041b7e6e22',
  type: 'INTEGER',
  value: '3',
};
const SITScheduleDest = {
  eTag: 'MjAyMS0wMy0xOFQwMTozMTo1MS4yNjY4M1o=',
  id: '5a3cf5bb-2bbf-4af6-8966-143d52d8f94b',
  key: 'SITScheduleDest',
  origin: 'SYSTEM',
  paymentServiceItemID: '28039a62-387d-479f-b50f-e0041b7e6e22',
  type: 'INTEGER',
  value: '3',
};
const ServicesScheduleOrigin = {
  eTag: 'MjAyMS0wMy0xOFQwMTozMTo1MS4yNjY4M1o=',
  id: 'cd37b2a6-ac7d-4c93-a148-ca67f7f67cff',
  key: 'ServicesScheduleOrigin',
  origin: 'SYSTEM',
  paymentServiceItemID: '28039a62-387d-479f-b50f-e0041b7e6e22',
  type: 'INTEGER',
  value: '3',
};
const ServicesScheduleDest = {
  eTag: 'MjAyMS0wMy0xOFQwMTozMTo1MS4yNjY4M1o=',
  id: '33751a6a-0a20-4ee2-9e46-ece2c1e997ba',
  key: 'ServicesScheduleDest',
  origin: 'SYSTEM',
  paymentServiceItemID: '28039a62-387d-479f-b50f-e0041b7e6e22',
  type: 'INTEGER',
  value: '3',
};
const RateAreaNonStdOrigin = {
  eTag: 'MjAyMS0wMy0xOFQwMTozMTo1MS4yNjY4M1o=',
  id: '0f564bdd-153c-44af-97ae-117b62339c02',
  key: 'RateAreaNonStdOrigin',
  origin: 'PRIME',
  paymentServiceItemID: '28039a62-387d-479f-b50f-e0041b7e6e22',
  type: 'STRING',
  value: '',
};
const RateAreaNonStdDest = {
  eTag: 'MjAyMS0wMy0xOFQwMTozMTo1MS4yNjY4M1o=',
  id: '73060cd4-15fd-4d6d-9d38-9f362c7da7af',
  key: 'RateAreaNonStdDest',
  origin: 'PRIME',
  paymentServiceItemID: '28039a62-387d-479f-b50f-e0041b7e6e22',
  type: 'STRING',
  value: '',
};
const PSILinehaulDom = {
  eTag: 'MjAyMS0wMy0xOFQwMTozMTo1MS4yNjY4M1o=',
  id: 'b051e180-4435-4dc1-b3b1-dbde8f1ba8ae',
  key: 'PSI_LinehaulDom',
  origin: 'SYSTEM',
  paymentServiceItemID: '28039a62-387d-479f-b50f-e0041b7e6e22',
  type: 'PaymentServiceItemUUID',
  value: '',
};
const PSILinehaulDomPrice = {
  eTag: 'MjAyMS0wMy0xOFQwMTozMTo1MS4yNjY4M1o=',
  id: 'eeabaff6-3b6c-4637-80a2-bfe3efad335c',
  key: 'PSI_LinehaulDomPrice',
  origin: 'SYSTEM',
  paymentServiceItemID: '28039a62-387d-479f-b50f-e0041b7e6e22',
  type: 'DECIMAL',
  value: '',
};
const PSILinehaulShort = {
  eTag: 'MjAyMS0wMy0xOFQwMTozMTo1MS4yNjY4M1o=',
  id: '30eb55c4-f8e2-4b83-8e40-3e734c292841',
  key: 'PSI_LinehaulShort',
  origin: 'SYSTEM',
  paymentServiceItemID: '28039a62-387d-479f-b50f-e0041b7e6e22',
  type: 'PaymentServiceItemUUID',
  value: '',
};
const PSILinehaulShortPrice = {
  eTag: 'MjAyMS0wMy0xOFQwMTozMTo1MS4yNjY4M1o=',
  id: '137a75a5-2d05-4c23-aef9-999c8e0418fc',
  key: 'PSI_LinehaulShortPrice',
  origin: 'SYSTEM',
  paymentServiceItemID: '28039a62-387d-479f-b50f-e0041b7e6e22',
  type: 'DECIMAL',
  value: '',
};
const PSIPriceDomOrigin = {
  eTag: 'MjAyMS0wMy0xOFQwMTozMTo1MS4yNjY4M1o=',
  id: '4f029437-2ba3-4d5d-b25f-c43c6aeb7077',
  key: 'PSI_PriceDomOrigin',
  origin: 'SYSTEM',
  paymentServiceItemID: '28039a62-387d-479f-b50f-e0041b7e6e22',
  type: 'PaymentServiceItemUUID',
  value: '',
};
const PSIPriceDomOriginPrice = {
  eTag: 'MjAyMS0wMy0xOFQwMTozMTo1MS4yNjY4M1o=',
  id: 'bfb33051-301a-4cd0-8271-e681e78ee095',
  key: 'PSI_PriceDomOriginPrice',
  origin: 'SYSTEM',
  paymentServiceItemID: '28039a62-387d-479f-b50f-e0041b7e6e22',
  type: 'DECIMAL',
  value: '10',
};
const PSIPriceDomDest = {
  eTag: 'MjAyMS0wMy0xOFQwMTozMTo1MS4yNjY4M1o=',
  id: '7ae62314-55f1-4fef-8b3d-c83edda9a4fd',
  key: 'PSI_PriceDomDest',
  origin: 'SYSTEM',
  paymentServiceItemID: '28039a62-387d-479f-b50f-e0041b7e6e22',
  type: 'PaymentServiceItemUUID',
  value: '',
};
const PSIPriceDomDestPrice = {
  eTag: 'MjAyMS0wMy0xOFQwMTozMTo1MS4yNjY4M1o=',
  id: '6d1e8e68-4dcc-4ece-b96d-61ee3dfd67ea',
  key: 'PSI_PriceDomDestPrice',
  origin: 'SYSTEM',
  paymentServiceItemID: '28039a62-387d-479f-b50f-e0041b7e6e22',
  type: 'DECIMAL',
  value: '',
};
const DimensionLength = {
  eTag: 'MjAyMS0wNy0yOVQyMDoxNTowMS4xNDA1MjZa',
  id: 'f5bb063e-38da-4c86-88ce-a6a328e70b92',
  key: 'DimensionLength',
  origin: 'PRIME',
  paymentServiceItemID: '28039a62-387d-479f-b50f-e0041b7e6e22',
  type: 'INTEGER',
  value: '3',
};
const DimensionWidth = {
  eTag: 'MjAyMS0wNy0yOVQyMDoxNTowMS4xNDA1MjZa',
  id: 'f5bb063e-38da-4c86-88ce-a6a328e70b92',
  key: 'DimensionWidth',
  origin: 'PRIME',
  paymentServiceItemID: '28039a62-387d-479f-b50f-e0041b7e6e22',
  type: 'INTEGER',
  value: '10',
};
const DimensionHeight = {
  eTag: 'MjAyMS0wNy0yOVQyMDoxNTowMS4xNDA1MjZa',
  id: 'f5bb063e-38da-4c86-88ce-a6a328e70b92',
  key: 'DimensionHeight',
  origin: 'PRIME',
  paymentServiceItemID: '28039a62-387d-479f-b50f-e0041b7e6e22',
  type: 'INTEGER',
  value: '6',
};
const NTSPackingFactor = {
  eTag: 'MjAyMS0wNy0yOVQyMDoxNTowMS4xNDA1MjZa',
  id: 'f5bb063e-38da-4c86-88ce-a6a328e70b92',
  key: 'NTSPackingFactor',
  origin: 'PRICER',
  paymentServiceItemID: '28039a62-387d-479f-b50f-e0041b7e6e22',
  type: 'DECIMAL',
  value: '1.35',
};
const StandaloneCrate = {
  eTag: 'MjAyMS0wNy0yOVQyMDoxNTowMS4xNDA1MjZa',
  id: 'f5bb063e-38da-4c86-88ce-a6a328e70b92',
  key: 'StandaloneCrate',
  origin: 'PRIME',
  paymentServiceItemID: '28039a62-387d-479f-b50f-e0041b7e6e22',
  type: 'BOOLEAN',
  value: 'FALSE',
};
const PerUnitCents = {
  eTag: 'MjAyMS0wMy0xOFQwMTozMTo1MS4zNTI1MDZb',
  id: 'b26fcc8f-2c06-4b00-8b51-4715a2eb0f34',
  key: 'ZipDestAddress',
  origin: 'SYSTEM',
  paymentServiceItemID: '28039a62-387d-479f-b50f-e0041b7e6e22',
  type: 'INTEGER',
  value: '1000',
};
const PortZip = {
  eTag: 'MjAyMS0wMy0xOFQwMTozMTo1MS4zNTI1MDZc',
  id: 'b26fcc8f-2c06-4b00-8b51-4715a2eb0f35',
  key: 'ZipDestAddress',
  origin: 'SYSTEM',
  paymentServiceItemID: '28039a62-387d-479f-b50f-e0041b7e6e22',
  type: 'STRING',
  value: '99505',
};
const ExternalCrate = {
  eTag: 'MjAyMS0wNy0yOVQyMDoxNTowMS4xNDA1MjZa',
  id: 'f5bb063e-38da-4c86-88ce-a6a328e70b92',
  key: 'ExternalCrate',
  origin: 'PRIME',
  paymentServiceItemID: '28039a62-387d-479f-b50f-e0041b7e6e22',
  type: 'BOOLEAN',
  value: 'FALSE',
};
const MarketOrigin = {
  eTag: 'MjAyMS0wNy0yOVQyMDoxNTowMS4xNDA1MjZa',
  id: 'f5bb063e-38da-4c86-88ce-a6a328e70b92',
  key: 'MarketOrigin',
  origin: 'PRIME',
  paymentServiceItemID: '28039a62-387d-479f-b50f-e0041b7e6e22',
  type: 'STRING',
  value: 'O',
};
const MarketDest = {
  eTag: 'MjAyMS0wNy0yOVQyMDoxNTowMS4xNDA1MjZa',
  id: 'f5bb063e-38da-4c86-88ce-a6a328e70b92',
  key: 'MarketDest',
  origin: 'PRIME',
  paymentServiceItemID: '28039a62-387d-479f-b50f-e0041b7e6e22',
  type: 'STRING',
  value: 'C',
};
const testParams = {
  DomesticLongHaul: [
    ContractCode,
    ContractYearName,
    EscalationCompounded,
    IsPeak,
    PriceRateOrFactor,
    ReferenceDate,
    ServiceAreaOrigin,
    WeightOriginal,
    WeightBilled,
    WeightEstimated,
    DistanceZip,
    ZipDestAddress,
    ZipPickupAddress,
  ],
  DomesticLongHaulWithReweigh: [
    ContractCode,
    ContractYearName,
    EscalationCompounded,
    IsPeak,
    PriceRateOrFactor,
    ReferenceDate,
    ServiceAreaOrigin,
    WeightOriginal,
    WeightBilled,
    WeightEstimated,
    WeightReweigh,
    DistanceZip,
    ZipDestAddress,
    ZipPickupAddress,
  ],
  DomesticLongHaulWeightWithAdjustedAndReweigh: [
    ContractCode,
    ContractYearName,
    EscalationCompounded,
    IsPeak,
    PriceRateOrFactor,
    ReferenceDate,
    ServiceAreaOrigin,
    WeightAdjusted,
    WeightOriginal,
    WeightBilled,
    WeightEstimated,
    WeightReweigh,
    DistanceZip,
    ZipDestAddress,
    ZipPickupAddress,
  ],
  DomesticLongHaulWithAdjusted: [
    ContractCode,
    ContractYearName,
    EscalationCompounded,
    IsPeak,
    PriceRateOrFactor,
    ReferenceDate,
    ServiceAreaOrigin,
    WeightAdjusted,
    WeightOriginal,
    WeightBilled,
    WeightEstimated,
    DistanceZip,
    ZipDestAddress,
    ZipPickupAddress,
  ],
  DomesticShortHaul: [
    ContractCode,
    ContractYearName,
    EscalationCompounded,
    IsPeak,
    PriceRateOrFactor,
    ReferenceDate,
    ServiceAreaOrigin,
    WeightOriginal,
    WeightBilled,
    WeightEstimated,
    DistanceZip,
    ZipDestAddress,
    ZipPickupAddress,
  ],
  DomesticOriginPrice: [
    ReferenceDate,
    WeightOriginal,
    WeightBilled,
    WeightEstimated,
    ZipPickupAddress,
    PriceRateOrFactor,
    ServiceAreaOrigin,
    EscalationCompounded,
    ContractYearName,
  ],
  DomesticDestinationPrice: [
    ReferenceDate,
    ServiceAreaDest,
    WeightOriginal,
    WeightBilled,
    WeightEstimated,
    ZipDestAddress,
    PriceRateOrFactor,
    EscalationCompounded,
    ContractYearName,
  ],
  DomesticOrigin1stSIT: [
    WeightOriginal,
    WeightBilled,
    WeightEstimated,
    PriceRateOrFactor,
    SITServiceAreaOrigin,
    ReferenceDate,
    ZipPickupAddress,
    EscalationCompounded,
    ContractYearName,
  ],
  InternationalOrigin1stSIT: [
    WeightOriginal,
    WeightBilled,
    WeightEstimated,
    PriceRateOrFactor,
    SITRateAreaOrigin,
    ReferenceDate,
    ZipPickupAddress,
    EscalationCompounded,
    ContractYearName,
  ],
  DomesticDestination1stSIT: [
    WeightOriginal,
    WeightBilled,
    WeightEstimated,
    PriceRateOrFactor,
    ServiceAreaDest,
    ReferenceDate,
    ZipDestAddress,
    EscalationCompounded,
    ContractYearName,
  ],
  InternationalDestination1stSIT: [
    WeightOriginal,
    WeightBilled,
    WeightEstimated,
    PriceRateOrFactor,
    SITRateAreaDest,
    ReferenceDate,
    ZipDestAddress,
    EscalationCompounded,
    ContractYearName,
  ],
  DomesticOriginAdditionalSIT: [
    NumberDaysSIT,
    ReferenceDate,
    SITServiceAreaOrigin,
    WeightOriginal,
    WeightBilled,
    WeightEstimated,
    ZipDestAddress,
    PriceRateOrFactor,
    IsPeak,
    NumberDaysSIT,
    EscalationCompounded,
    ContractYearName,
  ],
  InternationalOriginAdditionalSIT: [
    NumberDaysSIT,
    ReferenceDate,
    SITRateAreaOrigin,
    WeightOriginal,
    WeightBilled,
    WeightEstimated,
    ZipDestAddress,
    PriceRateOrFactor,
    IsPeak,
    NumberDaysSIT,
    EscalationCompounded,
    ContractYearName,
  ],
  DomesticDestinationAdditionalSIT: [
    NumberDaysSIT,
    ReferenceDate,
    ServiceAreaDest,
    WeightOriginal,
    WeightBilled,
    WeightEstimated,
    ZipDestAddress,
    PriceRateOrFactor,
    IsPeak,
    NumberDaysSIT,
    EscalationCompounded,
    ContractYearName,
  ],
  InternationalDestinationAdditionalSIT: [
    NumberDaysSIT,
    ReferenceDate,
    SITRateAreaDest,
    WeightOriginal,
    WeightBilled,
    WeightEstimated,
    ZipDestAddress,
    PriceRateOrFactor,
    IsPeak,
    NumberDaysSIT,
    EscalationCompounded,
    ContractYearName,
  ],
  DomesticOriginSITPickup: [
    ReferenceDate,
    ServiceAreaOrigin,
    SITScheduleOrigin,
    WeightOriginal,
    WeightBilled,
    WeightEstimated,
    DistanceZipSITOrigin,
    ZipSITOriginHHGOriginalAddress,
    ZipSITOriginHHGActualAddress,
    PriceRateOrFactor,
    ContractYearName,
    EscalationCompounded,
  ],
  InternationalOriginSITPickup: [
    ReferenceDate,
    WeightOriginal,
    WeightBilled,
    WeightEstimated,
    SITRateAreaOrigin,
    DistanceZipSITOrigin,
    ZipSITOriginHHGOriginalAddress,
    ZipSITOriginHHGActualAddress,
    PriceRateOrFactor,
    ContractYearName,
    EscalationCompounded,
  ],
  DomesticDestinationSITDelivery: [
    ContractCode,
    ContractYearName,
    DistanceZipSITDest,
    EscalationCompounded,
    PriceRateOrFactor,
    ReferenceDate,
    SITServiceAreaDest,
    SITScheduleDest,
    WeightOriginal,
    WeightBilled,
    WeightEstimated,
    ZipSITDestHHGOriginalAddress,
    ZipSITDestHHGFinalAddress,
  ],
  InternationalDestinationSITDelivery: [
    ContractCode,
    ContractYearName,
    DistanceZipSITDest,
    EscalationCompounded,
    PriceRateOrFactor,
    ReferenceDate,
    SITRateAreaDest,
    WeightOriginal,
    WeightBilled,
    WeightEstimated,
    ZipSITDestHHGOriginalAddress,
    ZipSITDestHHGFinalAddress,
  ],
  DomesticDestinationSITDeliveryLonghaul: [
    ContractCode,
    ContractYearName,
    DistanceZipSITDestLonghaul,
    EscalationCompounded,
    PriceRateOrFactor,
    ReferenceDate,
    SITServiceAreaDest,
    SITScheduleDest,
    WeightOriginal,
    WeightBilled,
    WeightEstimated,
    ZipSITDestHHGOriginalAddress,
    ZipSITDestHHGFinalAddress,
  ],
  InternationalDestinationSITDeliveryLonghaul: [
    ContractCode,
    ContractYearName,
    DistanceZipSITDestLonghaul,
    EscalationCompounded,
    PriceRateOrFactor,
    ReferenceDate,
    SITRateAreaDest,
    WeightOriginal,
    WeightBilled,
    WeightEstimated,
    ZipSITDestHHGOriginalAddress,
    ZipSITDestHHGFinalAddress,
  ],
  DomesticDestinationSITDeliveryMatchingZip3: [
    ContractCode,
    ContractYearName,
    DistanceZipSITDestMatchingZip3,
    EscalationCompounded,
    PriceRateOrFactor,
    ReferenceDate,
    SITServiceAreaDest,
    SITScheduleDest,
    WeightOriginal,
    WeightBilled,
    WeightEstimated,
    ZipSITDestHHGOriginalAddress,
    ZipSITDestHHGFinalAddressMatchingZip3,
  ],
  InternationalDestinationSITDeliveryMatchingZip3: [
    ContractCode,
    ContractYearName,
    DistanceZipSITDestMatchingZip3,
    EscalationCompounded,
    PriceRateOrFactor,
    ReferenceDate,
    SITRateAreaDest,
    WeightOriginal,
    WeightBilled,
    WeightEstimated,
    ZipSITDestHHGOriginalAddress,
    ZipSITDestHHGFinalAddressMatchingZip3,
  ],
  DomesticPacking: [
    ContractYearName,
    EscalationCompounded,
    PriceRateOrFactor,
    ReferenceDate,
    ServicesScheduleOrigin,
    WeightOriginal,
    WeightBilled,
    WeightEstimated,
    ZipPickupAddress,
  ],
  DomesticNTSPacking: [
    ContractYearName,
    EscalationCompounded,
    NTSPackingFactor,
    PriceRateOrFactor,
    ReferenceDate,
    ServicesScheduleOrigin,
    WeightOriginal,
    WeightBilled,
    WeightEstimated,
    ZipPickupAddress,
  ],
  DomesticUnpacking: [
    ContractYearName,
    EscalationCompounded,
    PriceRateOrFactor,
    ReferenceDate,
    ServiceAreaDest,
    ServicesScheduleDest,
    WeightOriginal,
    WeightBilled,
    WeightEstimated,
    ZipDestAddress,
  ],
  DomesticCrating: [
    ContractYearName,
    EscalationCompounded,
    PriceRateOrFactor,
    ReferenceDate,
    CubicFeetBilled,
    ServicesScheduleOrigin,
    ServiceAreaOrigin,
    ZipPickupAddress,
    DimensionWidth,
    DimensionHeight,
    DimensionLength,
    StandaloneCrate,
  ],
  DomesticUncrating: [
    ReferenceDate,
    EscalationCompounded,
    CubicFeetBilled,
    PriceRateOrFactor,
    ServicesScheduleDest,
    ServiceAreaDest,
    ZipDestAddress,
    DimensionWidth,
    DimensionHeight,
    DimensionLength,
  ],
  DomesticOriginShuttleService: [
    ReferenceDate,
    WeightBilled,
    WeightOriginal,
    WeightEstimated,
    ServicesScheduleOrigin,
    ServiceAreaOrigin,
    ZipPickupAddress,
    PriceRateOrFactor,
    EscalationCompounded,
  ],
  DomesticDestinationShuttleService: [
    ReferenceDate,
    ServiceAreaDest,
    WeightOriginal,
    WeightBilled,
    WeightEstimated,
    ZipDestAddress,
    PriceRateOrFactor,
    EscalationCompounded,
    ServicesScheduleDest,
    ContractCode,
  ],
  NonStandardHHG: [
    ReferenceDate,
    WeightBilled,
    WeightOriginal,
    WeightEstimated,
    RateAreaNonStdOrigin,
    RateAreaNonStdDest,
  ],
  NonStandardUB: [
    ReferenceDate,
    WeightBilled,
    WeightOriginal,
    WeightEstimated,
    RateAreaNonStdOrigin,
    RateAreaNonStdDest,
  ],
  FuelSurchage: [
    ActualPickupDate,
    DistanceZip,
    EIAFuelPrice,
    FSCPriceDifferenceInCents,
    FSCWeightBasedDistanceMultiplier,
    WeightBilled,
    WeightEstimated,
    ZipDestAddress,
    ZipPickupAddress,
  ],
  DomesticOriginSITFuelSurchage: [
    ActualPickupDate,
    DistanceZipSITOrigin,
    EIAFuelPrice,
    FSCPriceDifferenceInCents,
    FSCWeightBasedDistanceMultiplier,
    WeightBilled,
    WeightEstimated,
    ZipDestAddress,
    ZipPickupAddress,
    ZipSITOriginHHGOriginalAddress,
    ZipSITOriginHHGActualAddress,
  ],
  InternationalOriginSITFuelSurchage: [
    ActualPickupDate,
    DistanceZipSITOrigin,
    EIAFuelPrice,
    FSCPriceDifferenceInCents,
    FSCWeightBasedDistanceMultiplier,
    WeightBilled,
    WeightEstimated,
    ZipDestAddress,
    ZipPickupAddress,
    ZipSITOriginHHGOriginalAddress,
    ZipSITOriginHHGActualAddress,
  ],
  DomesticDestinationSITFuelSurchage: [
    ActualPickupDate,
    DistanceZipSITDest,
    EIAFuelPrice,
    FSCPriceDifferenceInCents,
    FSCWeightBasedDistanceMultiplier,
    WeightBilled,
    WeightEstimated,
    ZipDestAddress,
    ZipPickupAddress,
    ZipSITDestHHGOriginalAddress,
    ZipSITDestHHGFinalAddress,
  ],
  InternationalDestinationSITFuelSurchage: [
    ActualPickupDate,
    DistanceZipSITDest,
    EIAFuelPrice,
    FSCPriceDifferenceInCents,
    FSCWeightBasedDistanceMultiplier,
    WeightBilled,
    WeightEstimated,
    ZipDestAddress,
    ZipPickupAddress,
    ZipSITDestHHGOriginalAddress,
    ZipSITDestHHGFinalAddress,
  ],
  DomesticMobileHomeFactor: [
    PSILinehaulDom,
    PSILinehaulDomPrice,
    PSILinehaulShort,
    PSILinehaulShortPrice,
    PSIPriceDomOrigin,
    PSIPriceDomOriginPrice,
    PSIPriceDomDest,
    PSIPriceDomDestPrice,
  ],
  DomesticTowAwayBoatFactor: [
    PSILinehaulDom,
    PSILinehaulDomPrice,
    PSILinehaulShort,
    PSILinehaulShortPrice,
    PSIPriceDomOrigin,
    PSIPriceDomOriginPrice,
    PSIPriceDomDest,
    PSIPriceDomDestPrice,
  ],
  InternationalHHGPack: [
    ContractYearName,
    EscalationCompounded,
    PriceRateOrFactor,
    ReferenceDate,
    WeightOriginal,
    WeightBilled,
    WeightEstimated,
    ZipPickupAddress,
    PerUnitCents,
  ],
  InternationalHHGUnpack: [
    ContractYearName,
    EscalationCompounded,
    PriceRateOrFactor,
    ReferenceDate,
    WeightOriginal,
    WeightBilled,
    WeightEstimated,
    ZipPickupAddress,
    PerUnitCents,
  ],
  InternationalShippingAndLinehaul: [
    ContractYearName,
    EscalationCompounded,
    PriceRateOrFactor,
    ReferenceDate,
    WeightOriginal,
    WeightBilled,
    WeightEstimated,
    ZipPickupAddress,
    PerUnitCents,
  ],
  InternationalUBPrice: [
    ActualPickupDate,
    ContractYearName,
    EscalationCompounded,
    PriceRateOrFactor,
    ReferenceDate,
    WeightBilled,
  ],
  InternationalUBPackPrice: [
    ActualPickupDate,
    ContractYearName,
    EscalationCompounded,
    PriceRateOrFactor,
    ReferenceDate,
    WeightBilled,
    WeightOriginal,
    WeightEstimated,
  ],
  InternationalUBUnpackPrice: [
    ActualPickupDate,
    ContractYearName,
    EscalationCompounded,
    PriceRateOrFactor,
    ReferenceDate,
    WeightBilled,
    WeightOriginal,
    WeightEstimated,
  ],
  PortOfDebarkation: [
    ActualPickupDate,
    DistanceZip,
    EIAFuelPrice,
    FSCPriceDifferenceInCents,
    FSCWeightBasedDistanceMultiplier,
    WeightBilled,
    WeightEstimated,
    ZipDestAddress,
    PortZip,
  ],
  PortOfEmbarkation: [
    ActualPickupDate,
    DistanceZip,
    EIAFuelPrice,
    FSCPriceDifferenceInCents,
    FSCWeightBasedDistanceMultiplier,
    WeightBilled,
    WeightEstimated,
    ZipPickupAddress,
    PortZip,
  ],
  InternationalCrating: [
    ContractYearName,
    EscalationCompounded,
    PriceRateOrFactor,
    ReferenceDate,
    CubicFeetBilled,
    MarketOrigin,
    ServiceAreaOrigin,
    ZipPickupAddress,
    DimensionWidth,
    DimensionHeight,
    DimensionLength,
    StandaloneCrate,
    ExternalCrate,
  ],
  InternationalUncrating: [
    ReferenceDate,
    EscalationCompounded,
    CubicFeetBilled,
    PriceRateOrFactor,
    MarketDest,
    ServiceAreaDest,
    ZipDestAddress,
    DimensionWidth,
    DimensionHeight,
    DimensionLength,
  ],
  NTSPackingFactor,
  additionalCratingDataDCRT: {
    reServiceCode: 'DCRT',
    description: 'Grand piano',
    dimensions: [
      {
        type: 'CRATE',
        width: 100,
        height: 80,
        length: 50,
      },
    ],
  },
};

export default testParams;
