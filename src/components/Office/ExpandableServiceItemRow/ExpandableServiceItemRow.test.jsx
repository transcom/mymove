...
const serviceItem = {
  ...
  rejectionReason: 'Rejection reason test text',
  status: 'DENIED',
};
...
describe('Payment service items', () => {
  it('Displays rejected service item with rejection reason', async () => {
    ...
    expect(screen.getByText('Domestic linehaul')).toBeVisible();
    ...
    expect(screen.getByText('Billable weight (cwt)')).toBeVisible();
    expect(screen.getByText('16.5 cwt')).toBeVisible();
    expect(screen.getByText('Original: 2,000 lbs')).toBeVisible();
    expect(screen.getByText('Estimated: 1,500 lbs')).toBeVisible();

    expect(screen.getByText('Mileage')).toBeVisible();
    expect(screen.getByText('1,540')).toBeVisible();
    expect(screen.getByText('ZIP 62225 to ZIP 85309')).toBeVisible();

    expect(screen.getByText('Baseline linehaul price')).toBeVisible();
    expect(screen.getByText('3.148')).toBeVisible();
    expect(screen.getByText('Domestic non-peak')).toBeVisible();
    expect(screen.getByText('Origin service area: 456')).toBeVisible();
    expect(screen.getByText('Requested pickup: 02 Jan 2025')).toBeVisible();
  ...
  it('Displays accepted service item without displaying rejection reason markup', async () => {
    serviceItem.status = 'APPROVED';
    serviceItem.rejectionReason = null;
    render(
      <ExpandableServiceItemRow
        serviceItem={serviceItem}
        additionalServiceItemData={additionalServiceItemData}
    ...
    expect(screen.queryByText('Rejection reason test text')).toBeNull();
