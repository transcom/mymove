...
describe('MoveSearchForm', () => {
  ...
  describe('check validation', () => {
    it('can submit move code', async () => {
      ...
      userEvent.type(getByLabelText('Search'), '123456');
      await waitFor(() => {
    ...
    it('can submit DOD ID', async () => {
      ...
      userEvent.click(getByLabelText('DOD ID'));
      userEvent.type(getByLabelText('Search'), '1111111111');
      await waitFor(() => {
    ...
    it('can submit name', async () => {
      ...
      userEvent.click(getByLabelText('Customer Name'));
      userEvent.type(getByLabelText('Search'), 'Leo Spaceman');
      await waitFor(() => {
    ...
    it('submits move code when it is 6 characters', async () => {
      ...
      userEvent.click(getByLabelText('Move Code'));
      userEvent.type(getByLabelText('Search'), '123456');
      await waitFor(() => {
        expect(getByLabelText('Search')).toHaveValue('123456');
      });
    ...
    it('disables submit button when dod id is not 10 characters', async () => {
      const { getByLabelText, getByRole } = render(<MoveSearchForm onSubmit={jest.fn()} />);
      userEvent.click(getByLabelText('DOD ID'));
      userEvent.type(getByLabelText('Search'), '12345');
      await waitFor(() => {
        expect(getByLabelText('Search')).toHaveValue('12345');
      });
      expect(getByRole('button')).toBeDisabled();
      userEvent.type(getByLabelText('Search'), '67890');
      await waitFor(() => {
    ...
    it('disables submit button when move code is not 6 characters', async () => {
      ...
      expect(getByRole('button')).toBeDisabled();
      userEvent.type(getByLabelText('Search'), '6');
      await waitFor(() => {
        expect(getByLabelText('Search')).toHaveValue('123456');
      });
      expect(getByRole('button')).toBeEnabled();
