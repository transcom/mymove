export default {
  AIR_FORCE: 'AIR_FORCE',
  ARMY: 'ARMY',
  ARMY_CORPS_OF_ENGINEERS: 'ARMY_CORPS_OF_ENGINEERS',
  COAST_GUARD: 'COAST_GUARD',
  NAVY_AND_MARINES: 'NAVY_AND_MARINES',
  OFFICE_OF_SECRETARY_OF_DEFENSE: 'OFFICE_OF_SECRETARY_OF_DEFENSE',
};

export const DEPARTMENT_INDICATOR_OPTIONS = {
  AIR_FORCE: '57 Air Force and Space Force',
  ARMY: '21 Army',
  ARMY_CORPS_OF_ENGINEERS: '96 Army Corps of Engineers',
  COAST_GUARD: '70 Coast Guard',
  NAVY_AND_MARINES: '17 Navy and Marine Corps',
  OFFICE_OF_SECRETARY_OF_DEFENSE: '97 Office of Secretary of Defense and all Defense Agencies',
};

export const DEPARTMENT_INDICATOR_LABELS = {
  AIR_FORCE: 'Air Force and Space Force',
  ARMY: 'Army',
  ARMY_CORPS_OF_ENGINEERS: 'Army Corps of Engineers',
  COAST_GUARD: 'Coast Guard',
  NAVY_AND_MARINES: 'Navy and Marine Corps',
  OFFICE_OF_SECRETARY_OF_DEFENSE: 'Office of Secretary of Defense and all Defense Agencies',
};
