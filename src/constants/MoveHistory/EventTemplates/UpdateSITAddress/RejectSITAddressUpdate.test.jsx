...
describe('When given an update Destination SIT address history record', () => {
  const historyRecord = {
    action: a.UPDATE,
    changedValues: {
      office_remarks: 'Customer refuses to allow their goods to be stored in Florence',
      status: 'REJECTED',
    },
    context: [
      {
  ...
  it('displays the status of the request', () => {
    ...
    expect(screen.getByText(': Update request rejected')).toBeInTheDocument();
  });
  ...
  it('displays the office remarks', () => {
    const template = getTemplate(historyRecord);

    render(template.getDetails(historyRecord));
    expect(screen.getByText('Office remarks')).toBeInTheDocument();
    expect(screen.getByText(': Customer refuses to allow their goods to be stored in Florence')).toBeInTheDocument();
  });
});
