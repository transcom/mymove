...
describe('allows a SM to continue requesting a payment', function() {
  ...
  it('service should be able to continue requesting payment', () => {
    ...
    serviceMemberSubmitsWeightTicket('BOX_TRUCK', true);
...
function serviceMemberSubmitsWeightTicket(vehicleType, hasAnother = true, ordinal = null) {
  ...
  if (vehicleType === 'BOX_TRUCK' || vehicleType === 'PRO_GEAR') {
    cy.get('[name="vehicle_nickname"]').type('Nickname');
  } else if (vehicleType === 'CAR' || vehicleType === 'CAR_TRAILER') {
    cy.get('[name="vehicle_make"]').type('Make');
    cy.get('[name="vehicle_model"]').type('Model');
  }
