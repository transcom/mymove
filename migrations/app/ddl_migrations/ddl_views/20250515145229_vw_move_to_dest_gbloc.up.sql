...
DROP VIEW move_to_dest_gbloc;
CREATE OR REPLACE VIEW move_to_dest_gbloc
AS SELECT DISTINCT move_id,
    gbloc
   FROM ( SELECT sh.move_id,
            s.affiliation,
           ...
           FROM mto_shipments sh
             ...
             LEFT JOIN (SELECT a.id AS address_id,
                    ...
                    pctg_1.postal_code
                   FROM addresses a
                     JOIN postal_code_to_gblocs pctg_1 ON a.postal_code::text = pctg_1.postal_code::text) pctg_m ON pctg_m.address_id = sh.destination_address_id
             ...
             LEFT JOIN ( SELECT a.id AS address_id,
                   ...
                   FROM addresses a
                     ...
                     JOIN jppso_regions jr ON ga.jppso_regions_id = jr.id) pctg_oconus_bos ON pctg_oconus_bos.address_id = sh.destination_address_id AND
                CASE
             ...
             LEFT JOIN ( SELECT a.id AS address_id,
                    pctg_1.code::character varying AS gbloc,
                    ga.department_indicator
                   FROM addresses a
                     JOIN re_oconus_rate_areas ora ON a.us_post_region_cities_id = ora.us_post_region_cities_id
                     JOIN gbloc_aors ga ON ora.id = ga.oconus_rate_area_id
                     JOIN jppso_regions pctg_1 ON ga.jppso_regions_id = pctg_1.id) pctg_oconus ON pctg_oconus.address_id = sh.destination_address_id AND pctg_oconus.department_indicator IS NULL
          WHERE sh.deleted_at IS NULL) m;