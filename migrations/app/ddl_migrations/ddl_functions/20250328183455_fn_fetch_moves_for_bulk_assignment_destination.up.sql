...
INNER JOIN duty_locations AS origin_dl
 ON orders.origin_duty_location_id = origin_dl.id
...
AND v_gbloc = case when service_members.affiliation != 'MARINES'
and mto_shipments.shipment_type = 'HHG_OUTOF_NTS' THEN orders.gbloc
else
move_to_gbloc.gbloc
end
GROUP BY moves.id
