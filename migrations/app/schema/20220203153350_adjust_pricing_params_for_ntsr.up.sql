-- Historically, we've used the requested pickup date as the base date for determining
...
-- for the date used in billing.  The migration below adjusts the metadata and existing
-- pricing param records to account for that.

-- Add a new param key for the generic DateBilled parameter.
INSERT INTO service_item_param_keys(id, key, description, type, origin, created_at, updated_at)
VALUES ('5335e243-ab5b-4906-b84f-bd8c35ba64b3', 'DateBilled',
		'Date for determining peak vs. non-peak, escalation, etc.', 'DATE', 'SYSTEM', now(), now());

-- Adjust the service params and data as follows:
--   * add ActualPickupDate and DateBilled to the services that currently use RequestedPickupDate
--   * set RequestedPickupDate to be an optional parameter
--   * copy any existing RequestedPickupDate parameter values to a new DateBilled param
-- NOTE: Could do this more succinctly but we want to be explicit with what UUIDs get used
-- for metadata like in service_params.
DO $$
DECLARE
...
BEGIN
	...
	SELECT id INTO dateBilledUUID FROM service_item_param_keys WHERE key = 'DateBilled';
	...
	VALUES
		...
		('16bef588-d531-4ef9-b416-91a86873dd9b', (SELECT id FROM re_services WHERE code = 'DBHF'), dateBilledUUID, now(), now(), false),
		('7bc9fc4e-e40f-4f32-b7ca-da6e5b531c47', (SELECT id FROM re_services WHERE code = 'DBTF'), dateBilledUUID, now(), now(), false),
		('f36922eb-b890-4621-ac83-8cf751d28369', (SELECT id FROM re_services WHERE code = 'DCRT'), dateBilledUUID, now(), now(), false),
		('bfb87e6c-b46b-4e0e-9a16-555bdf819203', (SELECT id FROM re_services WHERE code = 'DCRTSA'), dateBilledUUID, now(), now(), false),
		('159b84dc-7851-4c0a-ad7a-9a10534c9e9b', (SELECT id FROM re_services WHERE code = 'DDASIT'), dateBilledUUID, now(), now(), false),
		('150b0054-80b3-4eb9-9352-01b715f97a99', (SELECT id FROM re_services WHERE code = 'DDDSIT'), dateBilledUUID, now(), now(), false),
		('f6dbdd56-db19-4556-89c3-388b05495fae', (SELECT id FROM re_services WHERE code = 'DDFSIT'), dateBilledUUID, now(), now(), false),
		('e553d347-b8ff-48bb-b1c2-1bd35572e007', (SELECT id FROM re_services WHERE code = 'DDP'), dateBilledUUID, now(), now(), false),
		('ceb80a91-2477-4d48-b14f-082d5ecd335a', (SELECT id FROM re_services WHERE code = 'DDSHUT'), dateBilledUUID, now(), now(), false),
		('ecf5c7b5-9672-441d-936f-58a26e0261c2', (SELECT id FROM re_services WHERE code = 'DLH'), dateBilledUUID, now(), now(), false),
		('9467f12e-9931-4dac-a9ec-6315dea616fa', (SELECT id FROM re_services WHERE code = 'DMHF'), dateBilledUUID, now(), now(), false),
		('a6d8e4d4-a7f1-4a26-84d4-b5cb0175579f', (SELECT id FROM re_services WHERE code = 'DNPK'), dateBilledUUID, now(), now(), false),
		('3fac6c0a-c8b3-43a6-b87b-b2d02f6906b6', (SELECT id FROM re_services WHERE code = 'DOASIT'), dateBilledUUID, now(), now(), false),
		('7e586c25-bfc6-45f9-9902-e3c7fc601fad', (SELECT id FROM re_services WHERE code = 'DOFSIT'), dateBilledUUID, now(), now(), false),
		('c58a9061-63ba-438a-bb8e-545680d25508', (SELECT id FROM re_services WHERE code = 'DOP'), dateBilledUUID, now(), now(), false),
		('26b445da-0fa6-4897-bae7-e28d9643d1a2', (SELECT id FROM re_services WHERE code = 'DOPSIT'), dateBilledUUID, now(), now(), false),
		('ea32d543-de4b-4cd7-ba09-df2736f2c907', (SELECT id FROM re_services WHERE code = 'DOSHUT'), dateBilledUUID, now(), now(), false),
		('d60d68e3-4d17-4921-97d3-e8688e0e08ab', (SELECT id FROM re_services WHERE code = 'DPK'), dateBilledUUID, now(), now(), false),
		('92877e3c-f91f-4160-83ed-e0103e2eddb9', (SELECT id FROM re_services WHERE code = 'DSH'), dateBilledUUID, now(), now(), false),
		('29fb5628-7223-4363-ab04-f122ac4f7269', (SELECT id FROM re_services WHERE code = 'DUCRT'), dateBilledUUID, now(), now(), false),
		('bf4cd708-ff3e-4f0e-83d0-8d6973abfd65', (SELECT id FROM re_services WHERE code = 'DUPK'), dateBilledUUID, now(), now(), false),
		('4808f444-ff7b-47dd-82f5-42aae21dc1cc', (SELECT id FROM re_services WHERE code = 'ICOLH'), dateBilledUUID, now(), now(), false),
		('80329018-7069-4986-ac12-8472b4a6f7ab', (SELECT id FROM re_services WHERE code = 'ICOUB'), dateBilledUUID, now(), now(), false),
		('7053edfc-718d-4f8c-b75d-6b129461e2ca', (SELECT id FROM re_services WHERE code = 'ICRT'), dateBilledUUID, now(), now(), false),
		('dec53bf4-c125-4889-9524-e3360300a366', (SELECT id FROM re_services WHERE code = 'ICRTSA'), dateBilledUUID, now(), now(), false),
		('5d3ad2e9-aacf-4f09-974c-bbca130b5fcf', (SELECT id FROM re_services WHERE code = 'IDASIT'), dateBilledUUID, now(), now(), false),
		('fd5d51f4-7260-4737-97ed-c148d331d8b2', (SELECT id FROM re_services WHERE code = 'IDDSIT'), dateBilledUUID, now(), now(), false),
		('cec13623-0164-4fb0-956d-c8d050099a52', (SELECT id FROM re_services WHERE code = 'IDFSIT'), dateBilledUUID, now(), now(), false),
		('31cc88e9-697b-490a-9817-18c3af17a4f5', (SELECT id FROM re_services WHERE code = 'IDSHUT'), dateBilledUUID, now(), now(), false),
		('fe1e641e-58f0-488e-b4a2-a800f6c391a1', (SELECT id FROM re_services WHERE code = 'IHPK'), dateBilledUUID, now(), now(), false),
		('f9c8e067-b9b7-445c-bb77-d4e4774f85a3', (SELECT id FROM re_services WHERE code = 'IHUPK'), dateBilledUUID, now(), now(), false),
		('5e459d0a-4386-4609-8ffb-d2acf17b1bed', (SELECT id FROM re_services WHERE code = 'IOASIT'), dateBilledUUID, now(), now(), false),
		('eb979d87-d4d0-4297-bc55-b3244ad3f20f', (SELECT id FROM re_services WHERE code = 'IOCLH'), dateBilledUUID, now(), now(), false),
		('c19a7945-624e-440f-845f-b12320e1489e', (SELECT id FROM re_services WHERE code = 'IOCUB'), dateBilledUUID, now(), now(), false),
		('db73d95a-98f1-4768-9cfe-dc0e9e52078b', (SELECT id FROM re_services WHERE code = 'IOFSIT'), dateBilledUUID, now(), now(), false),
		('d4335b53-4c8d-4be9-ba94-22c0871d79e5', (SELECT id FROM re_services WHERE code = 'IOOLH'), dateBilledUUID, now(), now(), false),
		('98db9320-48ae-471a-ae5c-f6d6882072b8', (SELECT id FROM re_services WHERE code = 'IOOUB'), dateBilledUUID, now(), now(), false),
		('59893605-5f7e-4c78-8f2f-98d8c31ad513', (SELECT id FROM re_services WHERE code = 'IOPSIT'), dateBilledUUID, now(), now(), false),
		('7f6a6b7d-fbed-44f9-bfdc-a66eec852f11', (SELECT id FROM re_services WHERE code = 'IOSHUT'), dateBilledUUID, now(), now(), false),
		('83720613-c616-422e-b68c-66ea1e07e24f', (SELECT id FROM re_services WHERE code = 'IUBPK'), dateBilledUUID, now(), now(), false),
		('b8d72116-6c77-4a96-ad70-f350de3448cd', (SELECT id FROM re_services WHERE code = 'IUBUPK'), dateBilledUUID, now(), now(), false),
		('f09c5e5b-6edd-4a7d-92e1-f8de81c1e753', (SELECT id FROM re_services WHERE code = 'IUCRT'), dateBilledUUID, now(), now(), false),
		('c1be4a58-1413-4848-a70d-318e9cbde3e4', (SELECT id FROM re_services WHERE code = 'NSTH'), dateBilledUUID, now(), now(), false),
		('9f592683-94b9-4383-8df6-071c8561dc5b', (SELECT id FROM re_services WHERE code = 'NSTUB'), dateBilledUUID, now(), now(), false);
