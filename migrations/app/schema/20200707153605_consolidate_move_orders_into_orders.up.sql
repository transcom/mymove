-- Lock the move_orders and orders tables so we don't have changes while
...
ALTER TABLE orders ADD COLUMN grade text;
ALTER TABLE orders ADD COLUMN entitlement_id uuid REFERENCES entitlements;
CREATE INDEX ON orders (entitlement_id);
ALTER TABLE orders ADD COLUMN origin_duty_station_id uuid REFERENCES duty_stations;
CREATE INDEX ON orders (origin_duty_station_id);

-- The order_type column in move_orders was an enum versus a varchar in the
-- orders table. If we prefer enum to varchar, then we'll need to update the
-- order_types to add the enum values used by the orders table, as defined in
-- the swagger internal.yaml. To make this a zero-downtime migration, we need
-- to perform 6 steps:
