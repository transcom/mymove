-- access_codes
COMMENT ON TABLE access_codes IS 'Holds access code needed to log into the customer app';
...
-- backup_contacts
COMMENT ON TABLE backup_contacts IS 'Holds all information reguarding a backup contact for the service member';
COMMENT ON COLUMN backup_contacts.service_member_id IS 'A foreign key that points the service_members table';
COMMENT ON COLUMN backup_contacts.name IS 'The name of the backup contact';
COMMENT ON COLUMN backup_contacts.email IS 'The eamil of the backup contact';
...
COMMENT ON COLUMN duty_station_names.duty_station_id IS 'A foreign key that points the duty stations table';
...
COMMENT ON COLUMN orders.created_at IS 'Date & time the orders was created';
COMMENT ON COLUMN orders.updated_at IS 'Date & time the orders was last updated';
COMMENT ON COLUMN orders.uploaded_orders_id IS 'A foreign key that points the document table';
...
-- notifications
COMMENT ON TABLE notifications IS 'Holds information about the notifications (emails) sent to service members';
COMMENT ON COLUMN notifications.service_member_id IS 'A foreign key that points the service_members table';
COMMENT ON COLUMN notifications.ses_message_id IS 'An uuid returned after a successful sent email message';
COMMENT ON COLUMN notifications.notification_type IS 'The type of notification sent to the service member including: move approved, move canceled, move reviewed, move submitted, and payment reminder';
COMMENT ON COLUMN notifications.created_at IS 'Date & time the notification was created';

-- personally_procured_moves
COMMENT ON TABLE personally_procured_moves IS 'Holds information about the personally procured moves - moves when service members move themselves';
COMMENT ON COLUMN personally_procured_moves.move_id IS 'A foreign key that points the moves table';
COMMENT ON COLUMN personally_procured_moves.size IS 'The size of a move: Large, Medimum, Small';
COMMENT ON COLUMN personally_procured_moves.weight_estimate IS 'The estimated weight the service member think they will move';
...
COMMENT ON COLUMN personally_procured_moves.pickup_postal_code IS 'The zip entered during the PPM setup process. This zip is used for pricing';
COMMENT ON COLUMN personally_procured_moves.additional_pickup_postal_code IS 'An additional zipcode if the service member needs to pick up items from another location - an office perhaps';
...
COMMENT ON COLUMN personally_procured_moves.days_in_storage IS 'Number of days that a service member will put their things in temporary storage - max of 90 days';
...
COMMENT ON COLUMN personally_procured_moves.has_additional_postal_code IS '';
COMMENT ON COLUMN personally_procured_moves.has_sit IS 'A boolean to determine if the user wants to use storeage in transit';
COMMENT ON COLUMN personally_procured_moves.has_requested_advance IS 'Date & time the personally procured move was last updated';
COMMENT ON COLUMN personally_procured_moves.advance_id IS '';
...
COMMENT ON COLUMN personally_procured_moves.planned_sit_max IS '';
COMMENT ON COLUMN personally_procured_moves.sit_max IS 'The maximum days that items can be in storage. Typcially 90 days';
...
COMMENT ON COLUMN personally_procured_moves.advance_worksheet_id IS 'A foreign key that points the documnts table';
...
COMMENT ON COLUMN personally_procured_moves.submit_date IS 'Date & time the service member submitted the ppm';
COMMENT ON COLUMN personally_procured_moves.approve_date IS 'Date & time the office user approved a service member''s ppm';
COMMENT ON COLUMN personally_procured_moves.reviewed_date IS 'Date & time the office user reviewed weight tickets and expences entered by the service member';
COMMENT ON COLUMN personally_procured_moves.has_pro_gear IS 'A boolean to determine if the service member says they pro-gear';
...
-- reimbursements
COMMENT ON TABLE reimbursements IS 'Holds inforamtion about reimbursements to a service member';
COMMENT ON COLUMN reimbursements.requested_amount IS 'The reimbursement amount the service member is requesting in cents';
COMMENT ON COLUMN reimbursements.method_of_receipt IS 'The way the service member wants to be reimbursed: MIL_PAY, OTHER_DD, GTCC';
...
-- service_members NOTE: using customer and not service member because that's what some existing comments use
COMMENT ON TABLE service_members IS 'Holds inforamtion about a customer';
COMMENT ON COLUMN service_members.user_id IS 'A foreign key that points the users table';
...
COMMENT ON COLUMN service_members.residential_address_id IS 'A foreign key that points the addresses table - containing the customer''s residential address';
COMMENT ON COLUMN service_members.backup_mailing_address_id IS 'A foreign key that points the addresses table - containing the customer''s backup mailing address';
COMMENT ON COLUMN service_members.created_at IS 'Date & time the service member was created';
COMMENT ON COLUMN service_members.updated_at IS 'Date & time the service member was last updated';
COMMENT ON COLUMN service_members.social_security_number_id IS 'A foreign key that points the social_security_numbers table';
COMMENT ON COLUMN service_members.duty_station_id IS 'A foreign key that points the addresses table - containing the customer''s current duty station';
...
COMMENT ON TABLE signed_certifications IS 'Holds inforamtion about when the customer signed the certificate';
COMMENT ON COLUMN signed_certifications.submitting_user_id IS 'A foreign key that points the users table';
COMMENT ON COLUMN signed_certifications.move_id IS 'A foreign key that points the moves table';
COMMENT ON COLUMN signed_certifications.certification_text IS 'The leaglese text the customer agrees to. Stored in: ppmPaymentLegal ';
...
COMMENT ON COLUMN signed_certifications.personally_procured_move_id IS 'A foreign key that points the personally_procured_moves table';
...
-- user_uploads
COMMENT ON TABLE user_uploads IS 'Holds information that joins the uploads to the corrisponding docuemnts and users';
COMMENT ON COLUMN user_uploads.document_id IS 'A foreign key that points the documents table';
COMMENT ON COLUMN user_uploads.uploader_id IS 'A foreign key that points the users table';
COMMENT ON COLUMN user_uploads.upload_id IS 'A foreign key that points the uploaded table';
