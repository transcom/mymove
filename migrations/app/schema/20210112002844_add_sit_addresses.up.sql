-- Column add
ALTER TABLE mto_service_items
    ADD COLUMN sit_destination_final_address_id uuid,
    ADD COLUMN sit_origin_original_address_id uuid,
    ADD COLUMN sit_origin_actual_address_id uuid,
    ADD CONSTRAINT mto_service_items_sit_destination_final_address_id_fkey FOREIGN KEY (sit_destination_final_address_id) REFERENCES addresses (id),
    ADD CONSTRAINT mto_service_items_sit_origin_original_address_id_fkey FOREIGN KEY (sit_origin_original_address_id) REFERENCES addresses (id),
    ADD CONSTRAINT mto_service_items_sit_origin_actual_address_id_fkey FOREIGN KEY (sit_origin_actual_address_id) REFERENCES addresses (id);

-- Column comments
COMMENT ON COLUMN mto_service_items.sit_destination_final_address_id IS 'Final delivery address for Destination SIT';
COMMENT ON COLUMN mto_service_items.sit_origin_original_address_id IS 'Original pickup address for Origin SIT';
COMMENT ON COLUMN mto_service_items.sit_origin_actual_address_id IS 'New pickup address for Origin SIT';
