...
COMMENT ON COLUMN dps_users.login_gov_email IS 'The login_gov email of the user.';
...
COMMENT ON COLUMN office_users.user_id IS 'The foreign key that points to the user id in the users table. This gets populated when the user first signs in via login_gov, which then creates the user in the users table, and the link is then made in this table.';
