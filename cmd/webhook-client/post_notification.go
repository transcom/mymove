...
import (
	...
	supportOperations "github.com/transcom/mymove/pkg/gen/supportclient/operations" // #TODO: Should this endpoint eventually live in its own file and not support?
...
func postNotification(cmd *cobra.Command, args []string) error {
	...
	message := v.GetString(MessageFlag)
	newNotification := &supportMessages.Notification{ //#TODO: Replicate this without using supportMessages
		Message: message,
	}

	notificationParams := supportOperations.NewPostNotificationParams() // #TODO: Replicate without using operations, need to find where we're getting context and httpclient
