...
func (o *UpdateReweighURL) Build() (*url.URL, error) {
	...
	var _path = "/mto-shipments/{mtoShipmentID}/reweighs/{reweighID}"

	mtoShipmentID := o.MtoShipmentID.String()
	if mtoShipmentID != "" {
		_path = strings.Replace(_path, "{mtoShipmentID}", mtoShipmentID, -1)
	} else {
		return nil, errors.New("mtoShipmentId is required on UpdateReweighURL")
	}

	reweighID := o.ReweighID.String()
	if reweighID != "" {
		_path = strings.Replace(_path, "{reweighID}", reweighID, -1)
	} else {
		return nil, errors.New("reweighId is required on UpdateReweighURL")
	}
...
func (o *UpdateReweighURL) BuildFull(scheme, host string) (*url.URL, error) {
	if scheme == "" {
		return nil, errors.New("scheme is required for a full url on UpdateReweighURL")
	}
	if host == "" {
		return nil, errors.New("host is required for a full url on UpdateReweighURL")
	}
