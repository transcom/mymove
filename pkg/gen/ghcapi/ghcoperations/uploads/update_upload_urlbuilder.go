...
func (o *UpdateUploadURL) Build() (*url.URL, error) {
	...
	var _path = "/uploads/{uploadID}/update"

	uploadID := o.UploadID.String()
	if uploadID != "" {
		_path = strings.Replace(_path, "{uploadID}", uploadID, -1)
	} else {
		return nil, errors.New("uploadId is required on UpdateUploadURL")
	}
...
func (o *UpdateUploadURL) BuildFull(scheme, host string) (*url.URL, error) {
	if scheme == "" {
		return nil, errors.New("scheme is required for a full url on UpdateUploadURL")
	}
	if host == "" {
		return nil, errors.New("host is required for a full url on UpdateUploadURL")
	}
