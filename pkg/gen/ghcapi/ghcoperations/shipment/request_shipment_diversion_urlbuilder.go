...
func (o *RequestShipmentDiversionURL) Build() (*url.URL, error) {
	...
	var _path = "/shipments/{shipmentID}/request-diversion"

	shipmentID := o.ShipmentID.String()
	if shipmentID != "" {
		_path = strings.Replace(_path, "{shipmentID}", shipmentID, -1)
	} else {
		return nil, errors.New("shipmentId is required on RequestShipmentDiversionURL")
	}
...
func (o *RequestShipmentDiversionURL) BuildFull(scheme, host string) (*url.URL, error) {
	if scheme == "" {
		return nil, errors.New("scheme is required for a full url on RequestShipmentDiversionURL")
	}
	if host == "" {
		return nil, errors.New("host is required for a full url on RequestShipmentDiversionURL")
	}
