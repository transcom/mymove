...
func (o *ApproveShipmentURL) Build() (*url.URL, error) {
	...
	var _path = "/shipments/{shipmentID}/approve"

	shipmentID := o.ShipmentID.String()
	if shipmentID != "" {
		_path = strings.Replace(_path, "{shipmentID}", shipmentID, -1)
	} else {
		return nil, errors.New("shipmentId is required on ApproveShipmentURL")
	}
...
func (o *ApproveShipmentURL) BuildFull(scheme, host string) (*url.URL, error) {
	if scheme == "" {
		return nil, errors.New("scheme is required for a full url on ApproveShipmentURL")
	}
	if host == "" {
		return nil, errors.New("host is required for a full url on ApproveShipmentURL")
	}
