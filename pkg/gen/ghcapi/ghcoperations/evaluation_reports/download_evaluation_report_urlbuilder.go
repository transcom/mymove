...
func (o *DownloadEvaluationReportURL) Build() (*url.URL, error) {
	...
	var _path = "/evaluation-reports/{reportID}/download"

	reportID := o.ReportID.String()
	if reportID != "" {
		_path = strings.Replace(_path, "{reportID}", reportID, -1)
	} else {
		return nil, errors.New("reportId is required on DownloadEvaluationReportURL")
	}
...
func (o *DownloadEvaluationReportURL) BuildFull(scheme, host string) (*url.URL, error) {
	if scheme == "" {
		return nil, errors.New("scheme is required for a full url on DownloadEvaluationReportURL")
	}
	if host == "" {
		return nil, errors.New("host is required for a full url on DownloadEvaluationReportURL")
	}
