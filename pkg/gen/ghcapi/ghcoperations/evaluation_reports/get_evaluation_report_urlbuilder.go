...
func (o *GetEvaluationReportURL) Build() (*url.URL, error) {
	...
	var _path = "/evaluation-reports/{reportID}"

	reportID := o.ReportID.String()
	if reportID != "" {
		_path = strings.Replace(_path, "{reportID}", reportID, -1)
	} else {
		return nil, errors.New("reportId is required on GetEvaluationReportURL")
	}
...
func (o *GetEvaluationReportURL) BuildFull(scheme, host string) (*url.URL, error) {
	if scheme == "" {
		return nil, errors.New("scheme is required for a full url on GetEvaluationReportURL")
	}
	if host == "" {
		return nil, errors.New("host is required for a full url on GetEvaluationReportURL")
	}
