...
func (o *GetEntitlementsURL) Build() (*url.URL, error) {
	...
	var _path = "/move-task-orders/{moveTaskOrderID}/entitlements"

	moveTaskOrderID := o.MoveTaskOrderID
	if moveTaskOrderID != "" {
		_path = strings.Replace(_path, "{moveTaskOrderID}", moveTaskOrderID, -1)
	} else {
		return nil, errors.New("moveTaskOrderId is required on GetEntitlementsURL")
	}
...
func (o *GetEntitlementsURL) BuildFull(scheme, host string) (*url.URL, error) {
	if scheme == "" {
		return nil, errors.New("scheme is required for a full url on GetEntitlementsURL")
	}
	if host == "" {
		return nil, errors.New("host is required for a full url on GetEntitlementsURL")
	}
