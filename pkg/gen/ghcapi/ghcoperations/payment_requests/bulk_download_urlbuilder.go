...
func (o *BulkDownloadURL) Build() (*url.URL, error) {
	...
	var _path = "/payment-requests/{paymentRequestID}/bulkDownload"

	paymentRequestID := o.PaymentRequestID
	if paymentRequestID != "" {
		_path = strings.Replace(_path, "{paymentRequestID}", paymentRequestID, -1)
	} else {
		return nil, errors.New("paymentRequestId is required on BulkDownloadURL")
	}
...
func (o *BulkDownloadURL) BuildFull(scheme, host string) (*url.URL, error) {
	if scheme == "" {
		return nil, errors.New("scheme is required for a full url on BulkDownloadURL")
	}
	if host == "" {
		return nil, errors.New("host is required for a full url on BulkDownloadURL")
	}
