...
func (o *GetShipmentsPaymentSITBalanceURL) Build() (*url.URL, error) {
	...
	var _path = "/payment-requests/{paymentRequestID}/shipments-payment-sit-balance"

	paymentRequestID := o.PaymentRequestID.String()
	if paymentRequestID != "" {
		_path = strings.Replace(_path, "{paymentRequestID}", paymentRequestID, -1)
	} else {
		return nil, errors.New("paymentRequestId is required on GetShipmentsPaymentSITBalanceURL")
	}
...
func (o *GetShipmentsPaymentSITBalanceURL) BuildFull(scheme, host string) (*url.URL, error) {
	if scheme == "" {
		return nil, errors.New("scheme is required for a full url on GetShipmentsPaymentSITBalanceURL")
	}
	if host == "" {
		return nil, errors.New("host is required for a full url on GetShipmentsPaymentSITBalanceURL")
	}
