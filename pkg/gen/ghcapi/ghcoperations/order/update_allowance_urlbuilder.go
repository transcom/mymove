...
func (o *UpdateAllowanceURL) Build() (*url.URL, error) {
	...
	var _path = "/orders/{orderID}/allowances"

	orderID := o.OrderID.String()
	if orderID != "" {
		_path = strings.Replace(_path, "{orderID}", orderID, -1)
	} else {
		return nil, errors.New("orderId is required on UpdateAllowanceURL")
	}
...
func (o *UpdateAllowanceURL) BuildFull(scheme, host string) (*url.URL, error) {
	if scheme == "" {
		return nil, errors.New("scheme is required for a full url on UpdateAllowanceURL")
	}
	if host == "" {
		return nil, errors.New("host is required for a full url on UpdateAllowanceURL")
	}
