...
func (o *PostRevisionToOrdersURL) Build() (*url.URL, error) {
	...
	var _path = "/orders/{uuid}"

	uuid := o.UUID.String()
	if uuid != "" {
		_path = strings.Replace(_path, "{uuid}", uuid, -1)
	} else {
		return nil, errors.New("uuid is required on PostRevisionToOrdersURL")
	}
...
func (o *PostRevisionToOrdersURL) BuildFull(scheme, host string) (*url.URL, error) {
	if scheme == "" {
		return nil, errors.New("scheme is required for a full url on PostRevisionToOrdersURL")
	}
	if host == "" {
		return nil, errors.New("host is required for a full url on PostRevisionToOrdersURL")
	}
