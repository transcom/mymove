...
func (o *GetElectronicOrdersTotalsURL) WithBasePath(bp string) *GetElectronicOrdersTotalsURL {
	o.SetBasePath(bp)
	return o
}
...
func (o *GetElectronicOrdersTotalsURL) Build() (*url.URL, error) {
	...
	for _, andFilterI := range o.AndFilter {
		andFilterIS := andFilterI
		if andFilterIS != "" {
			andFilterIR = append(andFilterIR, andFilterIS)
		}
	}

	andFilter := swag.JoinByFormat(andFilterIR, "")

	if len(andFilter) > 0 {
		qsv := andFilter[0]
		if qsv != "" {
			qs.Set("andFilter", qsv)
		}
	}
...
func (o *GetElectronicOrdersTotalsURL) String() string {
...
func (o *GetElectronicOrdersTotalsURL) BuildFull(scheme, host string) (*url.URL, error) {
	if scheme == "" {
		return nil, errors.New("scheme is required for a full url on GetElectronicOrdersTotalsURL")
	}
	if host == "" {
		return nil, errors.New("host is required for a full url on GetElectronicOrdersTotalsURL")
	}
