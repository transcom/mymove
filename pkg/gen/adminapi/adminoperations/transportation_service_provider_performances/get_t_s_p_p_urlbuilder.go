...
func (o *GetTSPPURL) Build() (*url.URL, error) {
	...
	if tsppID != "" {
		_path = strings.Replace(_path, "{tsppId}", tsppID, -1)
	} else {
		return nil, errors.New("tsppId is required on GetTSPPURL")
	}
...
func (o *GetTSPPURL) BuildFull(scheme, host string) (*url.URL, error) {
	if scheme == "" {
		return nil, errors.New("scheme is required for a full url on GetTSPPURL")
	}
	if host == "" {
		return nil, errors.New("host is required for a full url on GetTSPPURL")
	}
