...
type UpdateMTOShipmentStatus struct {
	...
	// status
	// Enum: [REJECTED APPROVED SUBMITTED CANCELLATION_REQUESTED DIVERSION_REQUESTED]
...
func init() {
	var res []string
	if err := json.Unmarshal([]byte(`["REJECTED","APPROVED","SUBMITTED","CANCELLATION_REQUESTED","DIVERSION_REQUESTED"]`), &res); err != nil {
		panic(err)
	}
	for _, v := range res {
