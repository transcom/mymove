...
func (o *SubmitPPMShipmentDocumentationURL) Build() (*url.URL, error) {
	...
	var _path = "/ppm-shipments/{ppmShipmentId}/submit-ppm-shipment-documentation"

	ppmShipmentID := o.PpmShipmentID.String()
	if ppmShipmentID != "" {
		_path = strings.Replace(_path, "{ppmShipmentId}", ppmShipmentID, -1)
	} else {
		return nil, errors.New("ppmShipmentId is required on SubmitPPMShipmentDocumentationURL")
	}
...
func (o *SubmitPPMShipmentDocumentationURL) BuildFull(scheme, host string) (*url.URL, error) {
	if scheme == "" {
		return nil, errors.New("scheme is required for a full url on SubmitPPMShipmentDocumentationURL")
	}
	if host == "" {
		return nil, errors.New("host is required for a full url on SubmitPPMShipmentDocumentationURL")
	}
