...
func (o *CreatePPMUploadURL) Build() (*url.URL, error) {
	...
	var _path = "/ppm-shipments/{ppmShipmentId}/uploads"

	ppmShipmentID := o.PpmShipmentID.String()
	if ppmShipmentID != "" {
		_path = strings.Replace(_path, "{ppmShipmentId}", ppmShipmentID, -1)
	} else {
		return nil, errors.New("ppmShipmentId is required on CreatePPMUploadURL")
	}
...
func (o *CreatePPMUploadURL) BuildFull(scheme, host string) (*url.URL, error) {
	if scheme == "" {
		return nil, errors.New("scheme is required for a full url on CreatePPMUploadURL")
	}
	if host == "" {
		return nil, errors.New("host is required for a full url on CreatePPMUploadURL")
	}
