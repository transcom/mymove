...
func (o *DeleteWeightTicketURL) Build() (*url.URL, error) {
	...
	var _path = "/ppm-shipments/{ppmShipmentId}/weight-ticket/{weightTicketId}"

	ppmShipmentID := o.PpmShipmentID.String()
	if ppmShipmentID != "" {
		_path = strings.Replace(_path, "{ppmShipmentId}", ppmShipmentID, -1)
	} else {
		return nil, errors.New("ppmShipmentId is required on DeleteWeightTicketURL")
	}

	weightTicketID := o.WeightTicketID.String()
	if weightTicketID != "" {
		_path = strings.Replace(_path, "{weightTicketId}", weightTicketID, -1)
	} else {
		return nil, errors.New("weightTicketId is required on DeleteWeightTicketURL")
	}
...
func (o *DeleteWeightTicketURL) BuildFull(scheme, host string) (*url.URL, error) {
	if scheme == "" {
		return nil, errors.New("scheme is required for a full url on DeleteWeightTicketURL")
	}
	if host == "" {
		return nil, errors.New("host is required for a full url on DeleteWeightTicketURL")
	}
