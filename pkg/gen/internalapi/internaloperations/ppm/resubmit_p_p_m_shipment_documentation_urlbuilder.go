...
func (o *ResubmitPPMShipmentDocumentationURL) Build() (*url.URL, error) {
	...
	var _path = "/ppm-shipments/{ppmShipmentId}/resubmit-ppm-shipment-documentation/{signedCertificationId}"

	ppmShipmentID := o.PpmShipmentID.String()
	if ppmShipmentID != "" {
		_path = strings.Replace(_path, "{ppmShipmentId}", ppmShipmentID, -1)
	} else {
		return nil, errors.New("ppmShipmentId is required on ResubmitPPMShipmentDocumentationURL")
	}

	signedCertificationID := o.SignedCertificationID.String()
	if signedCertificationID != "" {
		_path = strings.Replace(_path, "{signedCertificationId}", signedCertificationID, -1)
	} else {
		return nil, errors.New("signedCertificationId is required on ResubmitPPMShipmentDocumentationURL")
	}
...
func (o *ResubmitPPMShipmentDocumentationURL) BuildFull(scheme, host string) (*url.URL, error) {
	if scheme == "" {
		return nil, errors.New("scheme is required for a full url on ResubmitPPMShipmentDocumentationURL")
	}
	if host == "" {
		return nil, errors.New("host is required for a full url on ResubmitPPMShipmentDocumentationURL")
	}
