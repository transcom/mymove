...
func (o *DeleteUploadsURL) Build() (*url.URL, error) {
	...
	var uploadIdsIR []string
	for _, uploadIdsI := range o.UploadIds {
		uploadIdsIS := uploadIdsI.String()
		if uploadIdsIS != "" {
			uploadIdsIR = append(uploadIdsIR, uploadIdsIS)
		}
	}

	uploadIds := swag.JoinByFormat(uploadIdsIR, "")

	if len(uploadIds) > 0 {
		qsv := uploadIds[0]
		if qsv != "" {
			qs.Set("uploadIds", qsv)
		}
	}
...
func (o *DeleteUploadsURL) BuildFull(scheme, host string) (*url.URL, error) {
	if scheme == "" {
		return nil, errors.New("scheme is required for a full url on DeleteUploadsURL")
	}
	if host == "" {
		return nil, errors.New("host is required for a full url on DeleteUploadsURL")
	}
