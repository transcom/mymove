...
func (o *SubmitAmendedOrdersURL) WithBasePath(bp string) *SubmitAmendedOrdersURL {
	o.SetBasePath(bp)
	return o
}
...
func (o *SubmitAmendedOrdersURL) Build() (*url.URL, error) {
	...
	var _path = "/moves/{moveId}/submit_amended_orders"

	moveID := o.MoveID.String()
	if moveID != "" {
		_path = strings.Replace(_path, "{moveId}", moveID, -1)
	} else {
		return nil, errors.New("moveId is required on SubmitAmendedOrdersURL")
	}
...
func (o *SubmitAmendedOrdersURL) BuildFull(scheme, host string) (*url.URL, error) {
	if scheme == "" {
		return nil, errors.New("scheme is required for a full url on SubmitAmendedOrdersURL")
	}
	if host == "" {
		return nil, errors.New("host is required for a full url on SubmitAmendedOrdersURL")
	}
