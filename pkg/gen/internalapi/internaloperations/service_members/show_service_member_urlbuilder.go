...
func (o *ShowServiceMemberURL) Build() (*url.URL, error) {
	...
	var _path = "/service_members/{serviceMemberId}"

	serviceMemberID := o.ServiceMemberID.String()
	if serviceMemberID != "" {
		_path = strings.Replace(_path, "{serviceMemberId}", serviceMemberID, -1)
	} else {
		return nil, errors.New("serviceMemberId is required on ShowServiceMemberURL")
	}
...
func (o *ShowServiceMemberURL) BuildFull(scheme, host string) (*url.URL, error) {
	if scheme == "" {
		return nil, errors.New("scheme is required for a full url on ShowServiceMemberURL")
	}
	if host == "" {
		return nil, errors.New("host is required for a full url on ShowServiceMemberURL")
	}
