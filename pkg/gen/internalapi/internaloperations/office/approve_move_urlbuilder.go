...
func (o *ApproveMoveURL) Build() (*url.URL, error) {
	...
	var _path = "/moves/{moveId}/approve"

	moveID := o.MoveID.String()
	if moveID != "" {
		_path = strings.Replace(_path, "{moveId}", moveID, -1)
	} else {
		return nil, errors.New("moveId is required on ApproveMoveURL")
	}
...
func (o *ApproveMoveURL) BuildFull(scheme, host string) (*url.URL, error) {
	if scheme == "" {
		return nil, errors.New("scheme is required for a full url on ApproveMoveURL")
	}
	if host == "" {
		return nil, errors.New("host is required for a full url on ApproveMoveURL")
	}
