...
func (o *ShowOfficeOrdersURL) Build() (*url.URL, error) {
	...
	var _path = "/moves/{moveId}/orders"

	moveID := o.MoveID.String()
	if moveID != "" {
		_path = strings.Replace(_path, "{moveId}", moveID, -1)
	} else {
		return nil, errors.New("moveId is required on ShowOfficeOrdersURL")
	}
...
func (o *ShowOfficeOrdersURL) BuildFull(scheme, host string) (*url.URL, error) {
	if scheme == "" {
		return nil, errors.New("scheme is required for a full url on ShowOfficeOrdersURL")
	}
	if host == "" {
		return nil, errors.New("host is required for a full url on ShowOfficeOrdersURL")
	}
