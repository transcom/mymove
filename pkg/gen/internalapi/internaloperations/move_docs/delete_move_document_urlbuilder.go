...
func (o *DeleteMoveDocumentURL) Build() (*url.URL, error) {
	...
	var _path = "/move_documents/{moveDocumentId}"

	moveDocumentID := o.MoveDocumentID.String()
	if moveDocumentID != "" {
		_path = strings.Replace(_path, "{moveDocumentId}", moveDocumentID, -1)
	} else {
		return nil, errors.New("moveDocumentId is required on DeleteMoveDocumentURL")
	}
...
func (o *DeleteMoveDocumentURL) BuildFull(scheme, host string) (*url.URL, error) {
	if scheme == "" {
		return nil, errors.New("scheme is required for a full url on DeleteMoveDocumentURL")
	}
	if host == "" {
		return nil, errors.New("host is required for a full url on DeleteMoveDocumentURL")
	}
