...
func (o *ShowQueueURL) Build() (*url.URL, error) {
	...
	var _path = "/queues/{queueType}"

	queueType := o.QueueType
	if queueType != "" {
		_path = strings.Replace(_path, "{queueType}", queueType, -1)
	} else {
		return nil, errors.New("queueType is required on ShowQueueURL")
	}
...
func (o *ShowQueueURL) BuildFull(scheme, host string) (*url.URL, error) {
	if scheme == "" {
		return nil, errors.New("scheme is required for a full url on ShowQueueURL")
	}
	if host == "" {
		return nil, errors.New("host is required for a full url on ShowQueueURL")
	}
