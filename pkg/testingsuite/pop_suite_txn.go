...
func (suite *PopTestSuite) perTestTransactionDB() *pop.Connection {
	...
	if suite.preloadedTxnDb != nil {
		if suite.lowPrivConn == nil {
			...
			savepointID := newSavepointID(suite.preloadedTxnDb, testingName)
			err := suite.preloadedTxnDb.RawQuery("SAVEPOINT " + savepointID).Exec()
			if err != nil {
				log.Fatalf("Cannot start preload savepoint %s: %v", savepointID, err)
			}
			...
			suite.T().Cleanup(func() {
				...
				suite.lowPrivConn = nil
				err := suite.preloadedTxnDb.RawQuery("ROLLBACK TO SAVEPOINT " + savepointID).Exec()
				if err != nil {
					log.Fatalf("Preload savepoint `%s` rollback failed: %v", savepointID, err)
				}
			})
		}
...
// newSavepointID generates a unique savepoint ID that no longer than
// 64 characters
func newSavepointID(popConn *pop.Connection, testingName string) string {
	savepointSanitizer := regexp.MustCompile("[^a-zA-Z0-9_]")
	hasher := sha256.New()
	hasher.Write([]byte(testingName))
	sha := base64.URLEncoding.EncodeToString(hasher.Sum(nil))

	savepointID := savepointSanitizer.ReplaceAllString(
		popConn.ID+"_"+sha, "")
	if len(savepointID) > 64 {
		savepointID = savepointID[:64]
	}
	return savepointID
}
