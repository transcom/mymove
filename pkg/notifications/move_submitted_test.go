...
func (suite *NotificationSuite) TestMoveSubmitted() {
	...
	notification := NewMoveSubmitted(move.ID, false)
...
func (suite *NotificationSuite) TestMoveSubmittedIsGunSafeFeatureOnIsTrue() {
	isGunSafeFeatureOn := true
	move := factory.BuildMove(suite.DB(), nil, nil)
	notification := NewMoveSubmitted(move.ID, isGunSafeFeatureOn)
	...
	suite.NotEmpty(email.htmlBody)
	suite.NotEmpty(email.textBody)
	suite.Equal(notification.isGunSafeFeatureOn, isGunSafeFeatureOn)
	suite.Contains(email.textBody, "gun safe")
}
...
func (suite *NotificationSuite) TestMoveSubmittedHTMLTemplateRenderWithGunSafe() {
	approver := factory.BuildUser(nil, nil, nil)
	move := factory.BuildMove(suite.DB(), nil, nil)
	notification := NewMoveSubmitted(move.ID, true)
	...
	s := moveSubmittedEmailData{
		...
		ProvidesGovernmentCounseling:      true,
		OneSourceTransportationOfficeLink: OneSourceTransportationOfficeLink,
		IsGunSafeFeatureOn:                notification.isGunSafeFeatureOn,
	}
	expectedHTMLContent := `<p>*** DO NOT REPLY directly to this email ***</p>
...
Your standard weight allowance: 7,999 pounds. That is how much combined weight the government will pay for all movements between authorized locations under your orders. Be advised, if you are moving to an administratively restricted weight location this amount could be less.
...
func (suite *NotificationSuite) TestMoveSubmittedTextTemplateWithGunSafeRender() {
