...
func Test_ParseMigrationFilenameSQL(t *testing.T) {
	...
	m, err := ParseMigrationFilename("20190611004000_create_providers.sql")
	r.NoError(err)
	r.NotNil(m)
...
func Test_ParseMigrationFilenameFizzUpPostgres(t *testing.T) {
	...
	r.NotNil(err)
	r.Equal("invalid database type \"postgres\", expected \"all\" because fizz is database type independent", err.Error())
	r.Nil(m)
}
