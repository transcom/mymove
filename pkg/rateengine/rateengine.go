...
//ComputeLowestCostPPMoveWithLHDiscount uses zip codes and linehaul discount to make two calculations for the price of a PPM move - once with the pickup zip and once with the current duty station zip - and returns the lowest cost move.
func (re *RateEngine) ComputeLowestCostPPMMoveWithLHDiscount(weight unit.Pound, originPickupZip5 string, originDutyStationZip5 string, destinationZip5 string, distanceMilesFromOriginPickupZip int, distanceMilesFromOriginDutyStationZip int, date time.Time, daysInSit int, lhDiscount unit.DiscountRate, sitDiscount unit.DiscountRate) (cost CostComputation, err error) {
	costFromOriginPickupZip, err := re.ComputePPM(
		weight,
		originPickupZip5,
		destinationZip5,
		distanceMilesFromOriginPickupZip,
	...
	costFromOriginDutyStationZip, err := re.ComputePPM(
		weight,
		originDutyStationZip5,
		destinationZip5,
		distanceMilesFromOriginDutyStationZip,
