...
func buildOrderWithBuildType(db *pop.Connection, customs []Customization, traits []Trait, buildType orderBuildType) models.Order {
	...
	if db != nil {
		...
		if originDutyLocation.Address.PostalCode == "" {
			err := db.EagerPreload("Address.State", "Address.Country").Find(&originDutyLocation, originDutyLocation.ID)
			if err != nil {
		...
		if newDutyLocation.Address.PostalCode == "" {
			err := db.EagerPreload("Address.State", "Address.Country").Find(&newDutyLocation, newDutyLocation.ID)
			if err != nil {
