...
func (suite *HandlerSuite) TestGetMoveTaskOrder() {
	...
	suite.Run("Success - return all PaymentRequests fields assoicated with the getMoveTaskOrder", func() {
		...
		// the payment requests could be returned in any order, so
		// find the one we are looking for
		var paymentRequestPayload *primev2messages.PaymentRequest
		for i := range movePayload.PaymentRequests {
			if paymentRequest.ID.String() == movePayload.PaymentRequests[i].ID.String() {
				paymentRequestPayload = movePayload.PaymentRequests[i]
			}
		}
		suite.Require().NotNil(paymentRequestPayload)
	...
	suite.Run("Success - return all MTOServiceItemDomesticCrating fields assoicated with the getMoveTaskOrder", func() {
		...
		json, err := json.Marshal(serviceItemPayload)
		suite.NoError(err)
		payload := primev2messages.MTOServiceItemDomesticCrating{}
		err = payload.UnmarshalJSON(json)
		suite.NoError(err)
