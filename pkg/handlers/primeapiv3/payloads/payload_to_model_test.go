...
func (suite *PayloadsSuite) TestMTOServiceItemModel() {
	...
	DCRTSAServiceItem := &primev3messages.MTOServiceItemDomesticStandaloneCrating{
	...
	suite.Run("Fail -  Returns error for DCRTSA service item because of validation error", func() {
		...
		badDCRTSAServiceItem := &primev3messages.MTOServiceItemDomesticStandaloneCrating{
			ReServiceCode: &dcrtsaCode,
			Reason:        &reason,
			Description:   &description,
		}
...
func (suite *PayloadsSuite) TestPPMShipmentModelFromCreate() {
	...
	suite.Equal(address.PostalCode, model.PickupPostalCode)
	suite.Equal(address.PostalCode, model.DestinationPostalCode)
	suite.Equal(address2.PostalCode, *model.SecondaryPickupPostalCode)
	suite.Equal(address2.PostalCode, *model.SecondaryDestinationPostalCode)
