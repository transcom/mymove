...
func calculateMoveDatesFromMove(appCtx appcontext.AppContext, planner route.Planner, moveID uuid.UUID, moveDate time.Time) (dates.MoveDatesSummary, error) {
	...
	//if move.Orders.NewDutyLocation.Address.PostalCode[0:5] == move.Orders.ServiceMember.DutyStation.Address.PostalCode[0:5] {
	//	return summary, errors.New("NewDutyLocation must not have the same zip code as the original DutyStation")
	//}
