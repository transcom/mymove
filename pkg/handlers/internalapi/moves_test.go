...
func (suite *HandlerSuite) TestPatchMoveHandler() {
	...
	handler := PatchMoveHandler{handlers.NewHandlerConfig(suite.DB(), suite.TestLogger())}
...
func (suite *HandlerSuite) TestShowMoveHandler() {
	...
	showHandler := ShowMoveHandler{handlers.NewHandlerConfig(suite.DB(), suite.TestLogger())}
...
func (suite *HandlerSuite) TestSubmitMoveForApprovalHandler() {
	suite.Run("Submits ppm success", func() {
		...
		hConfig.SetNotificationSender(notifications.NewStubNotificationSender("milmovelocal", suite.TestLogger()))
		handler := SubmitMoveHandler{hConfig, moverouter.NewMoveRouter()}
		response := handler.Handle(params)
...
func (suite *HandlerSuite) TestShowMoveDatesSummaryHandler() {
	...
	showHandler := ShowMoveDatesSummaryHandler{hConfig}
...
func (suite *HandlerSuite) TestShowShipmentSummaryWorksheet() {
	...
	handler := ShowShipmentSummaryWorksheetHandler{hConfig}
	response := handler.Handle(params)
...
func (suite *HandlerSuite) TestSubmitAmendedOrdersHandler() {
	suite.Run("Submits move with amended orders for review", func() {
		...
		handler := SubmitAmendedOrdersHandler{hConfig, moverouter.NewMoveRouter()}
		response := handler.Handle(params)
