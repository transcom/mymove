...
func (h UpdateReviewedPaymentRequestStatusHandler) Handle(params paymentrequestop.UpdateReviewedPaymentRequestStatusParams) middleware.Responder {
	...
	if err != nil {
		...
		return paymentrequestop.NewUpdateReviewedPaymentRequestStatusInternalServerError().WithPayload(payloads.InternalServerError(handlers.FmtString(err.Error()), h.GetTraceID()))
	}
	...
	if err != nil {
		...
		return paymentrequestop.NewUpdateReviewedPaymentRequestStatusNotFound().WithPayload(payloads.ClientError(handlers.NotFoundMessage, msg, h.GetTraceID()))
	}
	...
	if err != nil {
		...
		case services.NotFoundError:
			return paymentrequestop.NewUpdateReviewedPaymentRequestStatusNotFound().WithPayload(payloads.ClientError(handlers.NotFoundMessage, err.Error(), h.GetTraceID()))
		case services.PreconditionFailedError:
			return paymentrequestop.NewUpdateReviewedPaymentRequestStatusPreconditionFailed().WithPayload(payloads.ClientError(handlers.PreconditionErrMessage, err.Error(), h.GetTraceID()))
		case services.ConflictError:
			return paymentrequestop.NewUpdateReviewedPaymentRequestStatusConflict().WithPayload(payloads.ClientError(handlers.ConflictErrMessage, err.Error(), h.GetTraceID()))
		default:
	...
	return paymentrequestop.NewUpdateReviewedPaymentRequestStatusOK().WithPayload(returnPayload)
}
	...
	if *sendToSyncada {
		...
		for _, pr := range paymentRequests {
			...
			if err != nil {
				switch err.(type) {
				case services.NotFoundError:
					return paymentrequestop.NewUpdateProcessedPaymentRequestStatusNotFound().WithPayload(payloads.ClientError(handlers.NotFoundMessage, err.Error(), h.GetTraceID()))
				case services.PreconditionFailedError:
					return paymentrequestop.NewUpdateProcessedPaymentRequestStatusPreconditionFailed().WithPayload(payloads.ClientError(handlers.PreconditionErrMessage, err.Error(), h.GetTraceID()))
				default:
					logger.Error(fmt.Sprintf("Error saving payment request status for ID: %s: %s", paymentRequestID, err))
					return paymentrequestop.NewUpdateProcessedPaymentRequestStatusInternalServerError().WithPayload(payloads.InternalServerError(handlers.FmtString(err.Error()), h.GetTraceID()))
				}
			}
