...
func (h IndexOfficesHandler) Handle(params officeop.IndexOfficesParams) middleware.Responder {
	...
	offices, err := h.OfficeListFetcher.FetchOfficeList(appCfg, queryFilters, nil, pagination, ordering)
	if err != nil {
		return handlers.ResponseForError(logger, err)
	}

	totalOfficesCount, err := h.OfficeListFetcher.FetchOfficeCount(appCfg, queryFilters)
	if err != nil {
		return handlers.ResponseForError(logger, err)
	}
