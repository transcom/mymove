...
func (h IndexMovesHandler) Handle(params moveop.IndexMovesParams) middleware.Responder {
	...
	moves, err := h.MoveListFetcher.FetchMoveList(appCfg, queryFilters, associations, pagination, ordering)
	if err != nil {
		return handlers.ResponseForError(logger, err)
	}
	movesCount := len(moves)

	totalMoveCount, err := h.MoveListFetcher.FetchMoveCount(appCfg, queryFilters)
	if err != nil {
		return handlers.ResponseForError(logger, err)
	}
...
func (h UpdateMoveHandler) Handle(params moveop.UpdateMoveParams) middleware.Responder {
	...
	updatedMove, err := h.MoveTaskOrderUpdater.ShowHide(appCfg, moveID, params.Move.Show)
	if err != nil {
		switch e := err.(type) {
