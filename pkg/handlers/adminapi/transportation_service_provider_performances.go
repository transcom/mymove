...
func (h IndexTSPPsHandler) Handle(params tsppop.IndexTSPPsParams) middleware.Responder {
	...
	tspps, err := h.TransportationServiceProviderPerformanceListFetcher.FetchTransportationServiceProviderPerformanceList(appCfg, queryFilters, nil, pagination, ordering)
	if err != nil {
		return handlers.ResponseForError(logger, err)
	}

	totalTSPPsCount, err := h.TransportationServiceProviderPerformanceListFetcher.FetchTransportationServiceProviderPerformanceCount(appCfg, queryFilters)
	if err != nil {
		return handlers.ResponseForError(logger, err)
	}
...
func (h GetTSPPHandler) Handle(params tsppop.GetTSPPParams) middleware.Responder {
	...
	tspp, err := h.TransportationServiceProviderPerformanceFetcher.FetchTransportationServiceProviderPerformance(appCfg, queryFilters)
	if err != nil {
		return handlers.ResponseForError(logger, err)
	}
