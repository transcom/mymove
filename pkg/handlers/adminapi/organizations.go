...
func (h IndexOrganizationsHandler) Handle(params organization.IndexOrganizationsParams) middleware.Responder {
	...
	organizations, err := h.OrganizationListFetcher.FetchOrganizationList(appCfg, queryFilters, nil, pagination, ordering)
	if err != nil {
		return handlers.ResponseForError(logger, err)
	}

	totalOrganizationsCount, err := h.OrganizationListFetcher.FetchOrganizationCount(appCfg, queryFilters)
	if err != nil {
		return handlers.ResponseForError(logger, err)
	}
