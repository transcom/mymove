...
func (h IndexOfficeUsersHandler) Handle(params officeuserop.IndexOfficeUsersParams) middleware.Responder {
	...
	err := h.ListFetcher.FetchRecordList(appCfg, &officeUsers, queryFilters, nil, pagination, ordering)
	if err != nil {
		return handlers.ResponseForError(logger, err)
	}

	totalOfficeUsersCount, err := h.ListFetcher.FetchRecordCount(appCfg, &officeUsers, queryFilters)
	if err != nil {
		return handlers.ResponseForError(logger, err)
	}
...
func (h GetOfficeUserHandler) Handle(params officeuserop.GetOfficeUserParams) middleware.Responder {
	...
	officeUser, err := h.OfficeUserFetcher.FetchOfficeUser(appCfg, queryFilters)
	if err != nil {
		return handlers.ResponseForError(logger, err)
	}
...
func (h CreateOfficeUserHandler) Handle(params officeuserop.CreateOfficeUserParams) middleware.Responder {
	...
	createdOfficeUser, verrs, err := h.OfficeUserCreator.CreateOfficeUser(appCfg, &officeUser, transportationIDFilter)
	if verrs != nil {
	...
	_, err = h.UserRoleAssociator.UpdateUserRoles(appCfg, *createdOfficeUser.UserID, updatedRoles)
	if err != nil {
...
func (h UpdateOfficeUserHandler) Handle(params officeuserop.UpdateOfficeUserParams) middleware.Responder {
	...
	updatedOfficeUser, verrs, err := h.OfficeUserUpdater.UpdateOfficeUser(appCfg, officeUserID, payload)
	...
	if updatedOfficeUser.UserID != nil && payload.Roles != nil {
		updatedRoles := rolesPayloadToModel(payload.Roles)
		_, err = h.UserRoleAssociator.UpdateUserRoles(appCfg, *updatedOfficeUser.UserID, updatedRoles)
		if err != nil {
			logger.Error("Error updating user roles", zap.Error(err))
			return officeuserop.NewUpdateOfficeUserInternalServerError()
		}
