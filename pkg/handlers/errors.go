...
func errorAsJSON(err Error, traceID string) []byte {

	// Turn Error Code into Error Message
	var title string
	switch err.Code() {
	case http.StatusMethodNotAllowed:
		title = MethodNotAllowedMessage
	default:
...
// generated by that package. We override it so that we can populate the
...
func ServeCustomError(rw http.ResponseWriter, r *http.Request, err error) {
	...
	fmt.Println("\n\nserving custom error from here", traceID)
	switch e := err.(type) {
	case *openapierrors.CompositeError:
