...
func (suite *HandlerSuite) TestGetMoveQueuesHandlerViewAsGbloc() {
	hqOfficeUser := factory.BuildOfficeUserWithRoles(suite.DB(), nil, []roles.RoleType{roles.RoleTypeHQ}) // Defaults to KKFA
	hqOfficeUser.User.Roles = append(hqOfficeUser.User.Roles, roles.Role{
		RoleType: roles.RoleTypeHQ,
	})

	tooOfficeUser := factory.BuildOfficeUserWithRoles(suite.DB(), nil, []roles.RoleType{roles.RoleTypeTOO}) // Defaults to KKFA
	tooOfficeUser.User.Roles = append(tooOfficeUser.User.Roles, roles.Role{
		RoleType: roles.RoleTypeTOO,
	})

	// Create an order with an origin duty location outside of office users' GBLOC
	excludedMove := factory.BuildMove(suite.DB(), []factory.Customization{
		{
			Model: models.TransportationOffice{
				Gbloc: "AGFM",
	...
	// Create moves within the office users' GBLOC
	factory.BuildMTOShipment(suite.DB(), []factory.Customization{
		{
			Model: models.Move{
				Status: models.MoveStatusServiceCounselingCompleted,
	...
	suite.Run("request as HQ user without specified view as GBLOC", func() {
		request := httptest.NewRequest("GET", "/queues/moves", nil)
		request = suite.AuthenticateOfficeRequest(request, hqOfficeUser)
		params := queues.GetMovesQueueParams{
			HTTPRequest: request,
		}
		handlerConfig := suite.HandlerConfig()
	...
	suite.Run("request as HQ user with specified view as GBLOC", func() {
		request := httptest.NewRequest("GET", "/queues/moves", nil)
		request = suite.AuthenticateOfficeRequest(request, hqOfficeUser)
		gblocAGFM := "AGFM"
		params := queues.GetMovesQueueParams{
			HTTPRequest: request,
			ViewAsGBLOC: &gblocAGFM,
		}
		handlerConfig := suite.HandlerConfig()
		mockUnlocker := movelocker.NewMoveUnlocker()
		handler := GetMovesQueueHandler{
	...
	suite.Run("request as non-HQ user without specified view as GBLOC", func() {
		request := httptest.NewRequest("GET", "/queues/moves", nil)
		request = suite.AuthenticateOfficeRequest(request, tooOfficeUser)
		params := queues.GetMovesQueueParams{
			HTTPRequest: request,
		}
		handlerConfig := suite.HandlerConfig()
	...
	// Handler should ignore ViewAsGBLOC param for non-HQ users
	suite.Run("request as non-HQ user with specified view as GBLOC", func() {
		request := httptest.NewRequest("GET", "/queues/moves", nil)
		request = suite.AuthenticateOfficeRequest(request, tooOfficeUser)
		gblocAGFM := "AGFM"
		params := queues.GetMovesQueueParams{
