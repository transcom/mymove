			...
			if len(indices) > 0 {
				...
				for _, index := range indices {
					...
					if serviceItems[index].ReService.Code == "CS" {
						price, displayParams, err = h.counselingPricer.Price(appCtx, serviceItems[index])
					} else if serviceItems[index].ReService.Code == "MS" {
						price, displayParams, err = h.moveManagementPricer.Price(appCtx, serviceItems[index])
					}
