...
func (suite *ModelSuite) TestPaymentRequestValidation() {
	...
	suite.Run("test empty PaymentServiceItem", func() {
		...
		expErrors := map[string][]string{
			"status":                 {"Status is not in the list [PENDING, REVIEWED, REVIEWED_AND_ALL_SERVICE_ITEMS_REJECTED, SENT_TO_GEX, RECEIVED_BY_GEX, PAID, EDI_ERROR, DEPRECATED, SEND_TO_TPPS_FAIL]."},
