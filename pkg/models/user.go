...
func GetFullServiceMemberProfile(db *pop.Connection, session *auth.Session) (*ServiceMember, error) {
	...
	err := db.Where("id = $1", session.ServiceMemberID).Eager("DutyStation.Address", "ResidentialAddress", "BackupMailingAddress", "Orders.NewDutyStation.Address", "Orders.UploadedOrders.Uploads", "Orders.Moves.PersonallyProcuredMoves.Advance", "BackupContacts", "User").All(&serviceMembers)
	if err != nil {
		return nil, err
	}
