...
func randMcNallyZip3Distance(appCtx appcontext.AppContext, fromZip3 string, toZip3 string) (int, error) {
	...
	if fromZip3 == toZip3 {
		...
		err := appCtx.DB().Where("from_zip3 = ? and to_zip3 = ?", toZip3, fromZip3).First(&distance)
		if err != nil {
			return 0, fmt.Errorf("failed to load distance in database fromZip3 (%s) toZip3 (%s): %w", toZip3, fromZip3, err)
		}
	} else {
		err := appCtx.DB().Where("from_zip3 = ? and to_zip3 = ?", fromZip3, toZip3).First(&distance)
		if err != nil {
