...
func (suite *ServiceParamValueLookupsSuite) TestServiceParamCache() {
	...
	suite.Run("weight billed shuttling and DLH", func() {
		...
		paramLookupService1, err := ServiceParamLookupInitialize(suite.TestAppContext(), suite.planner, subtestData.mtoServiceItem3.ID, subtestData.paymentRequest.ID, subtestData.paymentRequest.MoveTaskOrderID, &paramCache)
		suite.NoError(err)
		...
		paramLookupService2, err := ServiceParamLookupInitialize(suite.TestAppContext(), suite.planner, subtestData.mtoServiceItemShuttle.ID, subtestData.paymentRequest.ID, subtestData.paymentRequest.MoveTaskOrderID, &paramCache)
		suite.NoError(err)

		var shuttleEstimatedWeightStr string
		shuttleEstimatedWeightStr, err = paramLookupService2.ServiceParamValue(suite.TestAppContext(), subtestData.serviceItemParamKey1.Key)
		suite.FatalNoError(err)
	...
	suite.Run("cubic feet billed", func() {
		subtestData := suite.makeSubtestData()
		paramLookupService1, err := ServiceParamLookupInitialize(suite.TestAppContext(), suite.planner, subtestData.mtoServiceItemCrate1.ID, subtestData.paymentRequest.ID, subtestData.paymentRequest.MoveTaskOrderID, &paramCache)
		suite.NoError(err)

		var cubicFeet string
		cubicFeet, err = paramLookupService1.ServiceParamValue(suite.TestAppContext(), subtestData.serviceItemParamKey4.Key)
		suite.FatalNoError(err)
		suite.Equal("1029.33", cubicFeet)

		paramLookupService2, err := ServiceParamLookupInitialize(suite.TestAppContext(), suite.planner, subtestData.mtoServiceItemCrate2.ID, subtestData.paymentRequest.ID, subtestData.paymentRequest.MoveTaskOrderID, &paramCache)
		suite.NoError(err)

		cubicFeet, err = paramLookupService2.ServiceParamValue(suite.TestAppContext(), subtestData.serviceItemParamKey4.Key)
		suite.FatalNoError(err)
		suite.Equal("4", cubicFeet)
	})
