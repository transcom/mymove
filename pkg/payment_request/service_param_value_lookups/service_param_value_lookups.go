...
type ServiceItemParamKeyLookup interface {
	lookup(appCfg appconfig.AppConfig, keyData *ServiceItemParamKeyData) (string, error)
}
...
func ServiceParamLookupInitialize(
	...
	err := appCfg.DB().Eager("ReService").Find(&mtoServiceItem, mtoServiceItemID)
	if err != nil {
		switch err {
		case sql.ErrNoRows:
	...
	case models.ReServiceCodeDCRT, models.ReServiceCodeDUCRT, models.ReServiceCodeDCRTSA:
		err = appCfg.DB().Load(&mtoServiceItem, "Dimensions")
		if err != nil {
			return nil, err
		}
	...
	case models.ReServiceCodeDDASIT, models.ReServiceCodeDDDSIT, models.ReServiceCodeDDFSIT:
		...
		if mtoServiceItem.SITDestinationFinalAddressID != nil && *mtoServiceItem.SITDestinationFinalAddressID != uuid.Nil {
			err = appCfg.DB().Load(&mtoServiceItem, "SITDestinationFinalAddress")
			if err != nil {
				return nil, err
			}
	...
	if mtoServiceItem.ReService.Code != models.ReServiceCodeCS && mtoServiceItem.ReService.Code != models.ReServiceCodeMS {
		...
		err = appCfg.DB().Eager("PickupAddress", "DestinationAddress").Find(&mtoShipment, mtoServiceItem.MTOShipmentID)
		if err != nil {
			switch err {
			case sql.ErrNoRows:
				return nil, services.NewNotFoundError(mtoServiceItemID, "looking for MTOServiceItemID")
	...
	err = s.setLookup(appCfg, serviceItemCode, paramKey, ActualPickupDateLookup{
	...
	err = s.setLookup(appCfg, serviceItemCode, paramKey, RequestedPickupDateLookup{
		MTOShipment: mtoShipment,
	})
	if err != nil {
		return nil, err
	...
	err = s.setLookup(appCfg, serviceItemCode, paramKey, DistanceZip5Lookup{
	...
	err = s.setLookup(appCfg, serviceItemCode, paramKey, DistanceZip3Lookup{
		PickupAddress:      pickupAddress,
		DestinationAddress: destinationAddress,
	})
	if err != nil {
	...
	err = s.setLookup(appCfg, serviceItemCode, paramKey, FSCWeightBasedDistanceMultiplierLookup{
		MTOShipment: mtoShipment,
	})
	if err != nil {
		return nil, err
	...
	err = s.setLookup(appCfg, serviceItemCode, paramKey, WeightBilledActualLookup{
		MTOShipment: mtoShipment,
	})
	if err != nil {
		return nil, err
	...
	err = s.setLookup(appCfg, serviceItemCode, paramKey, WeightEstimatedLookup{
		MTOShipment: mtoShipment,
	})
	if err != nil {
		return nil, err
	...
	err = s.setLookup(appCfg, serviceItemCode, paramKey, WeightActualLookup{
		MTOShipment: mtoShipment,
	})
	if err != nil {
		return nil, err
	...
	err = s.setLookup(appCfg, serviceItemCode, paramKey, ZipAddressLookup{
	...
	err = s.setLookup(appCfg, serviceItemCode, paramKey, MTOAvailableToPrimeAtLookup{})
	if err != nil {
		return nil, err
	}

	paramKey = models.ServiceItemParamNameServiceAreaOrigin
	err = s.setLookup(appCfg, serviceItemCode, paramKey, ServiceAreaLookup{
		Address: pickupAddress,
	})
	if err != nil {
		return nil, err
	...
	err = s.setLookup(appCfg, serviceItemCode, paramKey, ContractCodeLookup{})
	if err != nil {
		return nil, err
	}

	paramKey = models.ServiceItemParamNameCubicFeetBilled
	err = s.setLookup(appCfg, serviceItemCode, paramKey, CubicFeetBilledLookup{
	...
	err = s.setLookup(appCfg, serviceItemCode, paramKey, PSILinehaulDomLookup{
		MTOShipment: mtoShipment,
	})
	if err != nil {
		return nil, err
	...
	err = s.setLookup(appCfg, serviceItemCode, paramKey, PSILinehaulDomPriceLookup{
		MTOShipment: mtoShipment,
	})
	if err != nil {
		return nil, err
	...
	err = s.setLookup(appCfg, serviceItemCode, paramKey, EIAFuelPriceLookup{
		MTOShipment: mtoShipment,
	})
	if err != nil {
		return nil, err
	...
	err = s.setLookup(appCfg, serviceItemCode, paramKey, ServicesScheduleLookup{
		Address: pickupAddress,
	})
	if err != nil {
		return nil, err
	...
	err = s.setLookup(appCfg, serviceItemCode, paramKey, SITScheduleLookup{
		Address: pickupAddress,
	})
	if err != nil {
		return nil, err
	...
	err = s.setLookup(appCfg, serviceItemCode, paramKey, NumberDaysSITLookup{
		MTOShipment: mtoShipment,
	})
	if err != nil {
		return nil, err
	...
	err = s.setLookup(appCfg, serviceItemCode, paramKey, ZipSITOriginHHGOriginalAddressLookup{
	...
	err = s.setLookup(appCfg, serviceItemCode, paramKey, ZipSITOriginHHGActualAddressLookup{
		ServiceItem: mtoServiceItem,
	})
	if err != nil {
		return nil, err
	...
	err = s.setLookup(appCfg, serviceItemCode, paramKey, DistanceZipSITDestLookup{
		DestinationAddress:      destinationAddress,
	...
	err = s.setLookup(appCfg, serviceItemCode, paramKey, DistanceZipSITOriginLookup{
		ServiceItem: mtoServiceItem,
	})
	if err != nil {
		return nil, err
	...
	err = s.setLookup(appCfg, serviceItemCode, paramKey, CubicFeetCratingLookup{
		Dimensions: serviceItemDimensions,
	})
	if err != nil {
		return nil, err
	...
	err = s.setLookup(appCfg, serviceItemCode, paramKey, DimensionHeightLookup{
		Dimensions: serviceItemDimensions,
	})
	if err != nil {
		return nil, err
	...
	err = s.setLookup(appCfg, serviceItemCode, paramKey, DimensionLengthLookup{
		Dimensions: serviceItemDimensions,
	})
	if err != nil {
		return nil, err
	...
	err = s.setLookup(appCfg, serviceItemCode, paramKey, DimensionWidthLookup{
		Dimensions: serviceItemDimensions,
	})
	if err != nil {
		return nil, err
...
func (s *ServiceItemParamKeyData) setLookup(appCfg appconfig.AppConfig, serviceItemCode models.ReServiceCode, paramKey models.ServiceItemParamName, lookup ServiceItemParamKeyLookup) error {
	useKey, err := s.serviceItemNeedsParamKey(appCfg, serviceItemCode, paramKey)
	if useKey && err == nil {
...
// database queries
func (s *ServiceItemParamKeyData) serviceItemNeedsParamKey(appCfg appconfig.AppConfig, serviceItemCode models.ReServiceCode, paramKey models.ServiceItemParamName) (bool, error) {
	if s.paramCache == nil {
	...
	useKey, err := s.paramCache.ServiceItemNeedsParamKey(appCfg, serviceItemCode, paramKey)
	if err != nil {
...
func (s *ServiceItemParamKeyData) ServiceParamValue(appCfg appconfig.AppConfig, key models.ServiceItemParamName) (string, error) {
	...
	if lookup, ok := s.lookups[key]; ok {
		value, err := lookup.lookup(appCfg, s)
		if err != nil {
