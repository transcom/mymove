...
func SendToSyncada(appCtx appcontext.AppContext, edi string, icn int64, gexSender services.GexSender, sftpSender services.SyncadaSFTPSender, sendEDIFile bool) (string, error) {
	...
	if (gexSender == nil) && (sftpSender == nil) {
		return syncadaFileName, fmt.Errorf("cannot send to Syncada, SendToSyncada() senders are nil")
	}
	if gexSender != nil {
		...
		if err != nil {
			...
			return syncadaFileName, fmt.Errorf("GEX sender encountered an error: %w", err)
		}
		if resp == nil {
			logger.Error("GEX Sender receieved no response from GEX")
			return syncadaFileName, fmt.Errorf("no response when sending EDI to GEX")
		}
		if resp.StatusCode != http.StatusOK {
			...
			return syncadaFileName, fmt.Errorf("received error response when sending EDI to GEX %v", resp)
		}
		logger.Info(
