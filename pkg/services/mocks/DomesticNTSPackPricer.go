...
func (_m *DomesticNTSPackPricer) Price(appCtx appcontext.AppContext, contractCode string, requestedPickupDate time.Time, weight unit.Pound, servicesScheduleOrigin int, isPPM bool) (unit.Cents, services.PricingDisplayParams, error) {
	...
	var r2 error
	if rf, ok := ret.Get(0).(func(appcontext.AppContext, string, time.Time, unit.Pound, int, bool) (unit.Cents, services.PricingDisplayParams, error)); ok {
		return rf(appCtx, contractCode, requestedPickupDate, weight, servicesScheduleOrigin, isPPM)
	}
...
func (_m *DomesticNTSPackPricer) PriceUsingParams(appCtx appcontext.AppContext, params models.PaymentServiceItemParams, featureFlagValues map[string]bool) (unit.Cents, services.PricingDisplayParams, error) {
...
// NewDomesticNTSPackPricer creates a new instance of DomesticNTSPackPricer. It also registers a testing interface on the mock and a cleanup function to assert the mocks expectations.
// The first argument is typically a *testing.T value.
func NewDomesticNTSPackPricer(t interface {
	mock.TestingT
	Cleanup(func())
}) *DomesticNTSPackPricer {
	mock := &DomesticNTSPackPricer{}
	mock.Mock.Test(t)
