// Code generated by mockery. DO NOT EDIT.

package mocks

import (
	http "net/http"

	mock "github.com/stretchr/testify/mock"
	services "github.com/transcom/mymove/pkg/services"

	testing "testing"
)

// GexSender is an autogenerated mock type for the GexSender type
type GexSender struct {
	mock.Mock
}

// SendToGex provides a mock function with given fields: channel, edi, filename
func (_m *GexSender) SendToGex(channel services.GEXChannel, edi string, filename string) (*http.Response, error) {
	ret := _m.Called(channel, edi, filename)

	var r0 *http.Response
	if rf, ok := ret.Get(0).(func(services.GEXChannel, string, string) *http.Response); ok {
		r0 = rf(channel, edi, filename)
	} else {
		if ret.Get(0) != nil {
			r0 = ret.Get(0).(*http.Response)
		}
	}

	var r1 error
	if rf, ok := ret.Get(1).(func(services.GEXChannel, string, string) error); ok {
		r1 = rf(channel, edi, filename)
	} else {
		r1 = ret.Error(1)
	}

	return r0, r1
}

// NewGexSender creates a new instance of GexSender. It also registers the testing.TB interface on the mock and a cleanup function to assert the mocks expectations.
func NewGexSender(t testing.TB) *GexSender {
	mock := &GexSender{}
	mock.Mock.Test(t)

	t.Cleanup(func() { mock.AssertExpectations(t) })

	return mock
}
