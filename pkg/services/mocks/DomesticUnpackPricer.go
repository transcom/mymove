...
func (_m *DomesticUnpackPricer) Price(appCtx appcontext.AppContext, contractCode string, requestedPickupDate time.Time, weight unit.Pound, servicesScheduleDest int, isPPM bool) (unit.Cents, services.PricingDisplayParams, error) {
	...
	if rf, ok := ret.Get(0).(func(appcontext.AppContext, string, time.Time, unit.Pound, int, bool) (unit.Cents, services.PricingDisplayParams, error)); ok {
		return rf(appCtx, contractCode, requestedPickupDate, weight, servicesScheduleDest, isPPM)
	}
...
type mockConstructorTestingTNewDomesticUnpackPricer interface {
	mock.TestingT
	Cleanup(func())
}
...
func NewDomesticUnpackPricer(t mockConstructorTestingTNewDomesticUnpackPricer) *DomesticUnpackPricer {
