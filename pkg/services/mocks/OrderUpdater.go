...
func (_m *OrderUpdater) UploadAmendedOrdersAsCustomer(logger *zap.Logger, userID uuid.UUID, orderID uuid.UUID, file io.ReadCloser, filename string, storer storage.FileStorer) (models.Upload, string, *validate.Errors, error) {
	ret := _m.Called(logger, userID, orderID, file, filename, storer)

	var r0 models.Upload
	if rf, ok := ret.Get(0).(func(*zap.Logger, uuid.UUID, uuid.UUID, io.ReadCloser, string, storage.FileStorer) models.Upload); ok {
	...
	var r1 string
	if rf, ok := ret.Get(1).(func(*zap.Logger, uuid.UUID, uuid.UUID, io.ReadCloser, string, storage.FileStorer) string); ok {
	...
	if rf, ok := ret.Get(2).(func(*zap.Logger, uuid.UUID, uuid.UUID, io.ReadCloser, string, storage.FileStorer) *validate.Errors); ok {
	...
	var r3 error
	if rf, ok := ret.Get(3).(func(*zap.Logger, uuid.UUID, uuid.UUID, io.ReadCloser, string, storage.FileStorer) error); ok {
		r3 = rf(logger, userID, orderID, file, filename, storer)
	} else {
		r3 = ret.Error(3)
	}
