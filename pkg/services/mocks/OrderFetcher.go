...
// ListOriginRequestsOrders provides a mock function with given fields: appCtx, officeUserID, role, params
func (_m *OrderFetcher) ListOriginRequestsOrders(appCtx appcontext.AppContext, officeUserID uuid.UUID, role roles.RoleType, params *services.ListOrderParams) ([]models.Move, int, error) {
	ret := _m.Called(appCtx, officeUserID, role, params)

	if len(ret) == 0 {
		panic("no return value specified for ListOriginRequestsOrders")
	}
