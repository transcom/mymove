...
// GetMoveForPaymentRequests provides a mock function with given fields: appCtx, moveID, eagerAssociations
func (_m *MoveTaskOrderFetcher) GetMoveForPaymentRequests(appCtx appcontext.AppContext, moveID uuid.UUID, eagerAssociations ...string) (*models.Move, error) {
	_va := make([]interface{}, len(eagerAssociations))
	for _i := range eagerAssociations {
		_va[_i] = eagerAssociations[_i]
	}
	var _ca []interface{}
	_ca = append(_ca, appCtx, moveID)
	_ca = append(_ca, _va...)
	ret := _m.Called(_ca...)
	...
	if rf, ok := ret.Get(0).(func(appcontext.AppContext, uuid.UUID, ...string) (*models.Move, error)); ok {
		return rf(appCtx, moveID, eagerAssociations...)
	}
	if rf, ok := ret.Get(0).(func(appcontext.AppContext, uuid.UUID, ...string) *models.Move); ok {
		r0 = rf(appCtx, moveID, eagerAssociations...)
	} else {
		if ret.Get(0) != nil {
			r0 = ret.Get(0).(*models.Move)
		}
	...
	if rf, ok := ret.Get(1).(func(appcontext.AppContext, uuid.UUID, ...string) error); ok {
		r1 = rf(appCtx, moveID, eagerAssociations...)
	} else {
		r1 = ret.Error(1)
	}
