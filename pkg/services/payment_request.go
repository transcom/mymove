...
type PaymentRequestCreator interface {
	CreatePaymentRequest(appCfg appconfig.AppConfig, paymentRequest *models.PaymentRequest) (*models.PaymentRequest, error)
}
...
type PaymentRequestListFetcher interface {
	FetchPaymentRequestList(appCfg appconfig.AppConfig, officeUserID uuid.UUID, params *FetchPaymentRequestListParams) (*models.PaymentRequests, int, error)
	FetchPaymentRequestListByMove(appCfg appconfig.AppConfig, officeUserID uuid.UUID, locator string) (*models.PaymentRequests, error)
}
...
type PaymentRequestFetcher interface {
	FetchPaymentRequest(appCfg appconfig.AppConfig, paymentRequestID uuid.UUID) (models.PaymentRequest, error)
}
...
type PaymentRequestReviewedFetcher interface {
	FetchReviewedPaymentRequest(appCfg appconfig.AppConfig) (models.PaymentRequests, error)
}
...
type PaymentRequestStatusUpdater interface {
	UpdatePaymentRequestStatus(appCfg appconfig.AppConfig, paymentRequest *models.PaymentRequest, eTag string) (*models.PaymentRequest, error)
}
...
type PaymentRequestUploadCreator interface {
	CreateUpload(appCfg appconfig.AppConfig, file io.ReadCloser, paymentRequestID uuid.UUID, userID uuid.UUID, filename string) (*models.Upload, error)
}
...
type PaymentRequestReviewedProcessor interface {
	ProcessReviewedPaymentRequest(appCfg appconfig.AppConfig)
	ProcessAndLockReviewedPR(appCfg appconfig.AppConfig, pr models.PaymentRequest) error
}
