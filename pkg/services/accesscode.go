...
type AccessCodeValidator interface {
	ValidateAccessCode(appCfg appconfig.AppConfig, code string, moveType models.SelectedMoveType) (*models.AccessCode, bool, error)
}
...
type AccessCodeFetcher interface {
	FetchAccessCode(appCfg appconfig.AppConfig, serviceMemberID uuid.UUID) (*models.AccessCode, error)
}
...
type AccessCodeClaimer interface {
	ClaimAccessCode(appCfg appconfig.AppConfig, code string, serviceMemberID uuid.UUID) (*models.AccessCode, *validate.Errors, error)
}
...
type AccessCodeListFetcher interface {
	FetchAccessCodeList(appCfg appconfig.AppConfig, filters []QueryFilter, associations QueryAssociations, pagination Pagination, ordering QueryOrder) (models.AccessCodes, error)
	FetchAccessCodeCount(appCfg appconfig.AppConfig, filters []QueryFilter) (int, error)
}
