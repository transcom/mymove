...
func (f fetchCustomer) FetchCustomer(appCtx appcontext.AppContext, customerID uuid.UUID) (*models.ServiceMember, error) {
	...
	if err := appCtx.DB().EagerPreload("ResidentialAddress.State", "ResidentialAddress.Country", "BackupMailingAddress.State", "BackupMailingAddress.Country", "BackupContacts").Find(customer, customerID); err != nil {
		switch err {
		case sql.ErrNoRows:
