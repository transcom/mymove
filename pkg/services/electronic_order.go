...
type ElectronicOrderListFetcher interface {
	FetchElectronicOrderList(appCfg appconfig.AppConfig, filters []QueryFilter, associations QueryAssociations, pagination Pagination, ordering QueryOrder) (models.ElectronicOrders, error)
	FetchElectronicOrderCount(appCfg appconfig.AppConfig, filters []QueryFilter) (int, error)
}
...
type ElectronicOrderCategoryCountFetcher interface {
	FetchElectronicOrderCategoricalCounts(appCfg appconfig.AppConfig, filters []QueryFilter, andFilters *[]QueryFilter) (map[interface{}]int, error)
}
