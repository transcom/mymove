...
func (suite *MoveTaskOrderServiceSuite) TestMoveTaskOrderHider_Hide() {
	setupTestData := func() (models.Move, models.MTOAgent) {
...
func (suite *MoveTaskOrderServiceSuite) TestMoveTaskOrderHider_isValidFakeServiceMember() {
	...
	setupTestData := func() []testdatagen.Assertions {
		...
		return invalidFakeData
	}

	for idx := range invalidReasons {
		suite.Run(fmt.Sprintf("invalid fake Service Member data %d", idx), func() {
			invalidFakeData := setupTestData()
			invalidData := invalidFakeData[idx]
...
func (suite *MoveTaskOrderServiceSuite) TestMoveTaskOrderHider_isValidFakeModelMTOShipment() {
	...
	for idx := range hideReasons {
		suite.Run(fmt.Sprintf("invalid fake MTOShipment data %d", idx), func() {
			invalidFakeData := setupTestData()
			invalidData := invalidFakeData[idx]
