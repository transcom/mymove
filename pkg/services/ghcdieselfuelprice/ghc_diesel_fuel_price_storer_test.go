...
func (suite *GHCDieselFuelPriceServiceSuite) Test_ghcDieselFuelPriceStorer() {
	...
	suite.Run("run storer for new publication date", func() {
		...
		suite.NoError(err)
		suite.Equal("2025-01-06T00:00:00Z", ghcDieselFuelPrice.PublicationDate.Format(time.RFC3339))
		suite.Equal(unit.Millicents(356100), ghcDieselFuelPrice.FuelPriceInMillicents)
