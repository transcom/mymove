...
func (suite *PaymentRequestServiceSuite) TestListShipmentPaymentSITBalance() {
	...
	suite.Run("international - IOASIT - returns only pending SIT status when there are no previous payments", func() {
		move, shipment := setUpShipmentWith120DaysOfAuthorizedSIT(suite.DB())
		...
		year, month, day := time.Now().Date()
		originEntryDate := time.Date(year, month, day-30, 0, 0, 0, 0, time.UTC)
		ioasit := factory.BuildMTOServiceItem(suite.DB(), []factory.Customization{
			{
				Model: models.MTOServiceItem{
					Status:       models.MTOServiceItemStatusApproved,
					SITEntryDate: &originEntryDate,
		...
		ioasitParam := factory.BuildPaymentServiceItemParam(suite.DB(), []factory.Customization{
			{
				Model: models.PaymentServiceItemParam{
					Value: originEntryDate.Format("2006-01-02"),
				},
		...
		paymentEndDate := originEntryDate.Add(time.Hour * 24 * 30)
		// Creates the SIT end date param for existing IOASIT payment request service item
		factory.BuildPaymentServiceItemParam(suite.DB(), []factory.Customization{
			{
				Model: models.PaymentServiceItemParam{
					Value: paymentEndDate.Format("2006-01-02"),
			...
			{
				Model:    ioasitParam.PaymentServiceItem,
				LinkOnly: true,
			},
			{
				Model:    paymentRequest,
		...
		// Creates the NumberDaysSIT param for existing IOASIT payment request service item
		factory.BuildPaymentServiceItemParam(suite.DB(), []factory.Customization{
			{
				Model: models.PaymentServiceItemParam{
					Value: "30",
		...
		// To understand this, use a date range calculator and set start date
		// to AUG 12 2024, end date to DEC 9 2024, and select to "Include end date in calculation"
		suite.Equal(ioasit.SITEntryDate.AddDate(0, 0, 119).String(), pendingSITBalance.TotalSITEndDate.UTC().String())
	})

	suite.Run("international - IDASIT - returns only pending SIT status when there are no previous payments", func() {
		move, shipment := setUpShipmentWith120DaysOfAuthorizedSIT(suite.DB())
		...
		year, month, day := time.Now().Date()
		originEntryDate := time.Date(year, month, day-30, 0, 0, 0, 0, time.UTC)
		idasit := factory.BuildMTOServiceItem(suite.DB(), []factory.Customization{
			{
				Model: models.MTOServiceItem{
					Status:       models.MTOServiceItemStatusApproved,
					SITEntryDate: &originEntryDate,
		...
		idasitParam := factory.BuildPaymentServiceItemParam(suite.DB(), []factory.Customization{
			{
				Model: models.PaymentServiceItemParam{
					Value: originEntryDate.Format("2006-01-02"),
				},
		...
		factory.BuildPaymentServiceItemParam(suite.DB(), []factory.Customization{
			...
			{
				Model:    idasitParam.PaymentServiceItem,
				LinkOnly: true,
			},
			{
				Model:    paymentRequest,
		...
		// Creates the NumberDaysSIT param for existing IDASIT payment request service item
		factory.BuildPaymentServiceItemParam(suite.DB(), []factory.Customization{
			{
				Model: models.PaymentServiceItemParam{
					Value: "30",
		...
		// To understand this, use a date range calculator and set start date
		// to AUG 12 2024, end date to DEC 9 2024, and select to "Include end date in calculation"
		suite.Equal(idasit.SITEntryDate.AddDate(0, 0, 119).String(), pendingSITBalance.TotalSITEndDate.UTC().String())
	})
}
