...
func (f *paymentRequestListFetcher) FetchPaymentRequestList(appCfg appconfig.AppConfig, officeUserID uuid.UUID, params *services.FetchPaymentRequestListParams) (*models.PaymentRequests, int, error) {

	gblocFetcher := officeuser.NewOfficeUserGblocFetcher()
	gbloc, gblocErr := gblocFetcher.FetchGblocForOfficeUser(appCfg, officeUserID)
	if gblocErr != nil {
	...
	for i := range paymentRequests {
		...
		loadErr := appCfg.DB().Load(&paymentRequests[i].MoveTaskOrder.Orders, "ServiceMember")
		if loadErr != nil {
			return nil, 0, err
		}
	}
...
func (f *paymentRequestListFetcher) FetchPaymentRequestListByMove(appCfg appconfig.AppConfig, officeUserID uuid.UUID, locator string) (*models.PaymentRequests, error) {
	gblocFetcher := officeuser.NewOfficeUserGblocFetcher()
	gbloc, gblocErr := gblocFetcher.FetchGblocForOfficeUser(appCfg, officeUserID)
	if gblocErr != nil {
