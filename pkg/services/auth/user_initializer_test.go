...
func (suite *UserInitializerSuite) TestUserInitializerExistingUser() {
	// Given a combo office/TSP user
	user := dataHelper(suite.DB(), dataHelperParams{
		isOfficeUser: true,
		isTspUser:    true,
	...
	// Make an existing user that has the same email address as "new" office and TSP user models
	existingUser := testdatagen.MakeUser(suite.DB(), testdatagen.Assertions{
		User: models.User{
			LoginGovEmail: user.Email,
			LoginGovUUID:  uuid.Must(uuid.FromString(user.UserID)),
		},
	})
	...
	identity, err := initializer.InitializeUser(user)
	suite.NoError(err)
	suite.Equal(existingUser.ID, identity.ID)
	suite.NotNil(identity.OfficeUserID)
	suite.NotNil(identity.TspUserID)
}
