...
func (suite *UsersRolesServiceSuite) TestAssociateUserRolesRemove() {
	...
	_, _, err = urc.UpdateUserRoles(suite.AppContextForTest(), *officeUser.UserID, origRoleTypes)
	suite.NoError(err)

	// soft delete role1 and add role2
	newRoleTypes := []roles.RoleType{role2.RoleType}
	_, _, err = urc.UpdateUserRoles(suite.AppContextForTest(), *officeUser.UserID, newRoleTypes)
	suite.NoError(err)
...
func (suite *UsersRolesServiceSuite) TestAssociateUserRolesRemoveAllRoles() {
	...
	_, _, err = urc.UpdateUserRoles(suite.AppContextForTest(), *officeUser.UserID, []roles.RoleType{role1.RoleType, role2.RoleType})
	suite.NoError(err)
	ur := models.UsersRoles{}
	n, err := suite.DB().Count(&ur)
	suite.NoError(err)
	...
	_, _, err = urc.UpdateUserRoles(suite.AppContextForTest(), *officeUser.UserID, []roles.RoleType{})
	suite.NoError(err)
