...
func (suite *UsersRolesServiceSuite) TestGenerateUsersRoles() {
	id1, _ := uuid.NewV4()
	role1 := roles.Role{
		ID:       id1,
		RoleType: "role1",
	...
	allRoles := roles.Roles{role1, role2, role3}

	filterOn := []roles.RoleType{"role1", "role2"}

	userID, _ := uuid.NewV4()

	urs := UserRoles(allRoles, filterOn, userID)

	expected := []models.UsersRoles{{
		UserID: userID,
		RoleID: role1.ID},
		{
			UserID: userID,
