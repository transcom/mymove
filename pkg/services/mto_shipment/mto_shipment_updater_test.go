...
func (suite *MTOShipmentServiceSuite) TestMTOShipmentUpdater() {
	...
	suite.Run("Etag is stale", func() {
		...
		_, err := mtoShipmentUpdaterCustomer.UpdateMTOShipmentV1(suite.AppContextWithSessionForTest(&session), &mtoShipment, eTag)
		suite.Error(err)
	...
	suite.Run("404 Not Found Error - shipment can only be created for service member associated with the current session", func() {
		...
		updatedShipment, err := mtoShipmentUpdaterCustomer.UpdateMTOShipmentV1(session, &shipment, eTag)
		suite.Error(err)
	...
	suite.Run("If-Unmodified-Since is equal to the updated_at date", func() {
		...
		session := auth.Session{}
		updatedMTOShipment, err := mtoShipmentUpdaterCustomer.UpdateMTOShipmentV1(suite.AppContextWithSessionForTest(&session), &mtoShipment, eTag)
	...
	suite.Run("Updater can handle optional queries set as nil", func() {
		...
		session := auth.Session{}
		updatedMTOShipment, err := mtoShipmentUpdaterCustomer.UpdateMTOShipmentV1(suite.AppContextWithSessionForTest(&session), &mtoShipment2, eTag)
	...
	suite.Run("Successfully remove a secondary pickup address", func() {
		...
		session := auth.Session{}
		newShipment, err := mtoShipmentUpdaterCustomer.UpdateMTOShipmentV1(suite.AppContextWithSessionForTest(&session), &updatedShipment, eTag)
	...
	suite.Run("Successful update to all address fields", func() {
		...
		session := auth.Session{}
		updatedShipment, err := mtoShipmentUpdaterCustomer.UpdateMTOShipmentV1(suite.AppContextWithSessionForTest(&session), updatedShipment, eTag)
	...
	suite.Run("Can update destination address type on shipment", func() {
		...
		newShipment, err := mtoShipmentUpdaterOffice.UpdateMTOShipmentV1(suite.AppContextWithSessionForTest(&session), &updatedShipment, eTag)
	...
	suite.Run("Successfully update MTO Agents", func() {
		...
		session := auth.Session{}
		updatedMTOShipment, err := mtoShipmentUpdaterCustomer.UpdateMTOShipmentV1(suite.AppContextWithSessionForTest(&session), &updatedShipment, eTag)
	...
	suite.Run("Successfully add storage facility to shipment", func() {
		...
		session.Roles = append(session.Roles, too.User.Roles...)
		updatedMTOShipment, err := mtoShipmentUpdaterOffice.UpdateMTOShipmentV1(suite.AppContextWithSessionForTest(&session), &updatedShipment, eTag)
	...
	suite.Run("Successfully edit storage facility on shipment", func() {
		...
		session.Roles = append(session.Roles, too.User.Roles...)
		updatedShipment, err := mtoShipmentUpdaterOffice.UpdateMTOShipmentV1(suite.AppContextWithSessionForTest(&session), &newShipment, eTag)
		suite.Require().NoError(err)
	...
	suite.Run("Successfully divert a shipment and transition statuses", func() {
		...
		session := auth.Session{}
		updatedShipment, err := mtoShipmentUpdaterCustomer.UpdateMTOShipmentV1(suite.AppContextWithSessionForTest(&session), &shipmentInput, eTag)
	...
	suite.Run("Updating a shipment does not nullify ApprovedDate", func() {
		...
		session := auth.Session{}
		newShipment, err := mtoShipmentUpdaterPrime.UpdateMTOShipmentV1(suite.AppContextWithSessionForTest(&session), &updatedShipment, eTag)
	...
	suite.Run("Prime not able to update an existing prime estimated weight", func() {
		...
		session := auth.Session{}
		_, err = mtoShipmentUpdaterPrime.UpdateMTOShipmentV1(suite.AppContextWithSessionForTest(&session), &updatedShipment, eTag)
...
func (suite *MTOShipmentServiceSuite) TestUpdateShipmentEstimatedWeightMoveExcessWeight() {
	...
	suite.Run("Updating the shipment estimated weight will flag excess weight on the move and transitions move status", func() {
		...
		session := auth.Session{}
		_, err := mtoShipmentUpdaterPrime.UpdateMTOShipmentV1(suite.AppContextWithSessionForTest(&session), &primeShipment, etag.GenerateEtag(primeShipment.UpdatedAt))
		suite.NoError(err)
	...
	suite.Run("Skips calling check excess weight if estimated weight was not provided in request", func() {
		...
		session := auth.Session{}
		_, err := mockedUpdater.UpdateMTOShipmentV1(suite.AppContextWithSessionForTest(&session), &primeShipment, etag.GenerateEtag(primeShipment.UpdatedAt))
		suite.NoError(err)
...
func (suite *MTOShipmentServiceSuite) TestUpdateShipmentNullableFields() {
	...
	suite.Run("tacType and sacType are set to null when empty string is passed in", func() {
		...
		session.Roles = append(session.Roles, too.User.Roles...)
		_, err := mockedUpdater.UpdateMTOShipmentV1(suite.AppContextWithSessionForTest(&session), requestedUpdate, etag.GenerateEtag(ntsMove.MTOShipments[0].UpdatedAt))
		suite.NoError(err)
		suite.Equal(nil, nil)
		suite.Equal(nil, nil)
	})
	...
	suite.Run("tacType and sacType are updated when passed in", func() {
		...
		session.Roles = append(session.Roles, too.User.Roles...)
		updatedMtoShipment, err := mockedUpdater.UpdateMTOShipmentV1(suite.AppContextWithSessionForTest(&session), requestedUpdate, etag.GenerateEtag(shipment.UpdatedAt))
		suite.NoError(err)
