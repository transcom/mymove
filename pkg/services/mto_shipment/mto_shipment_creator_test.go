...
func (suite *MTOShipmentServiceSuite) TestCreateMTOShipment() {
	...
	suite.Run("Test unsuccessful diversion from shipment that itself is a diversion as well but without the DivertedFromShipmentID", func() {
		...
		for _, tt := range testCases {
			...
			// ! BUT, it is a diversion without a DivertedFromShipmentID
			// ! So, if a diversion is made off of this one and it tries to
			// ! specify a DivertedFromShipmentID, it should fail
			// ! It should fail because a DivertedFromShipmentID can only come from a diversion
			...
			childFromParentDivertedShipment := factory.BuildMTOShipment(nil, []factory.Customization{
				...
				},
				{
					Model: models.MTOShipment{
						ShipmentType:           tt.shipmentType,
						Diversion:              true,
						DivertedFromShipmentID: &createdDivertedParentShipment.ID,
					},
				},
			}, nil)
