...
`
		...
		suite.Error(err)
		suite.Equal(err.Error(), "bla")
	})

	// 	suite.T().Run("successfully updates a payment request status after processing a valid EDI997", func(t *testing.T) {
	// 		sample997EDIString := `
	// ISA*00*0084182369*00*0000000000*ZZ*MILMOVE        *12*8004171844     *201002*1504*U*00401*000009998*0*T*|
	// GS*SI*8004171844*MILMOVE*20210217*152945*220001*X*004010
	// ST*997*0001
	// AK1*SI*100001251
	// AK2*858*0001
