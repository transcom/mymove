...
func findCounselingOffice(appCtx appcontext.AppContext, dutyLocationID uuid.UUID) (models.TransportationOffices, error) {
	...
	with counseling_offices as (
		SELECT transportation_offices.id, transportation_offices.name, transportation_offices.address_id as counseling_address, substring(addresses.postal_code, 1,3 ) as origin_zip
	...
	SELECT counseling_offices.id, counseling_offices.name
		...
		ORDER BY zip3_distances.distance_miles asc`
