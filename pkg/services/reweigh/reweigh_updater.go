...
func (f *reweighUpdater) updateDivertedShipmentReweigh(appCtx appcontext.AppContext, reweigh *models.Reweigh, eTag string, checks ...reweighValidator) (*models.Reweigh, error) {
	...
	txnError := appCtx.NewTransaction(func(txnAppCtx appcontext.AppContext) error {
		shipmentIDs, lowestWeight, err := f.getDivertedShipmentIDsAndLowestWeight(txnAppCtx, reweigh)
		if err != nil {
			return err
		}
...
func (f *reweighUpdater) getDivertedShipmentIDsAndLowestWeight(appCtx appcontext.AppContext, reweigh *models.Reweigh) ([]uuid.UUID, *unit.Pound, error) {
