...
func (suite *GHCRateEngineServiceSuite) TestPriceDomesticLinehaul() {
	...
	suite.T().Run("success without PaymentServiceItemParams", func(t *testing.T) {
		priceCents, _, err := linehaulServicePricer.Price(isShortHaul, testdatagen.DefaultContractCode, dlhRequestedPickupDate, dlhTestDistance, dlhTestWeight, dlhTestServiceArea)
		suite.NoError(err)
		suite.Equal(dlhPriceCents, priceCents)
	})
	...
	suite.T().Run("not finding a rate record", func(t *testing.T) {
		_, _, err := linehaulServicePricer.Price(isShortHaul, "BOGUS", dlhRequestedPickupDate, dlhTestDistance, dlhTestWeight, dlhTestServiceArea)
		suite.Error(err)
	})

	suite.T().Run("validation errors", func(t *testing.T) {
		// No contract code
		_, _, err := linehaulServicePricer.Price(isShortHaul, "", dlhRequestedPickupDate, dlhTestDistance, dlhTestWeight, dlhTestServiceArea)
		suite.Error(err)
		suite.Equal("ContractCode is required", err.Error())

		// No requested pickup date
		_, _, err = linehaulServicePricer.Price(isShortHaul, testdatagen.DefaultContractCode, time.Time{}, dlhTestDistance, dlhTestWeight, dlhTestServiceArea)
		suite.Error(err)
		...
		// No service area
		_, _, err = linehaulServicePricer.Price(isShortHaul, testdatagen.DefaultContractCode, dlhRequestedPickupDate, dlhTestDistance, dlhTestWeight, "")
		suite.Error(err)
