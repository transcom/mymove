...
func (p domesticUnpackPricer) Price(appCfg appconfig.AppConfig, contractCode string, requestedPickupDate time.Time, weight unit.Pound, servicesScheduleDest int) (unit.Cents, services.PricingDisplayParams, error) {
	// Validate parameters
	...
	domOtherPrice, err := fetchDomOtherPrice(appCfg, contractCode, models.ReServiceCodeDUPK, servicesScheduleDest, isPeakPeriod)
...
func (p domesticUnpackPricer) PriceUsingParams(appCfg appconfig.AppConfig, params models.PaymentServiceItemParams) (unit.Cents, services.PricingDisplayParams, error) {
	...
	return p.Price(appCfg, contractCode, requestedPickupDate, unit.Pound(weightBilledActual), servicesScheduleDest)
}
