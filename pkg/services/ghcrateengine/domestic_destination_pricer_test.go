...
func (suite *GHCRateEngineServiceSuite) TestPriceDomesticDestinationWithServiceItemParamsBadData() {
	...
	suite.Run("failure during pricing bubbles up", func() {
		suite.T().Skip("this test will only pass if disable min weight is false on the pricer")
...
func (suite *GHCRateEngineServiceSuite) TestPriceDomesticDestination() {
	...
	suite.Run("validation errors", func() {
		...
		// Negative weight
		_, _, err = pricer.Price(suite.AppContextForTest(), testdatagen.DefaultContractCode, requestedPickupDate, -250, ddpTestServiceArea, !isPPM, false)
		suite.Error(err)
