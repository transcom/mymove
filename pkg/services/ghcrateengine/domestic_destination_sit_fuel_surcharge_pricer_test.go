...
func (suite *GHCRateEngineServiceSuite) TestPriceDomesticDestinationFuelSurcharge() {
	DomesticDestinationFuelSurchargePricer := NewDomesticDestinationFuelSurchargePricer()
	...
	suite.Run("success using PaymentServiceItemParams", func() {
		...
		priceCents, displayParams, err := DomesticDestinationFuelSurchargePricer.PriceUsingParams(suite.AppContextForTest(), paymentServiceItem.PaymentServiceItemParams)
		suite.NoError(err)
	...
	suite.Run("success without PaymentServiceItemParams", func() {
		isPPM := false
		priceCents, _, err := DomesticDestinationFuelSurchargePricer.Price(suite.AppContextForTest(), ddsfscActualPickupDate, ddsfscTestDistance, ddsfscTestWeight, ddsfscWeightDistanceMultiplier, ddsfscFuelPrice, isPPM)
		suite.NoError(err)
		suite.Equal(ddsfscPriceCents, priceCents)
	})
	...
	suite.Run("success without PaymentServiceItemParams when shipment is PPM with < 500 lb weight", func() {
		isPPM := true
		priceCents, _, err := DomesticDestinationFuelSurchargePricer.Price(suite.AppContextForTest(), ddsfscActualPickupDate, ddsfscTestDistance, unit.Pound(250), ddsfscWeightDistanceMultiplier, ddsfscFuelPrice, isPPM)
		suite.NoError(err)
		suite.Equal(ddsfscPriceCents, priceCents)
	})
	...
	suite.Run("success using PaymentServiceItemParams with below minimum weight for a PPM shipment", func() {
		...
		priceCents, _, err := DomesticDestinationFuelSurchargePricer.PriceUsingParams(suite.AppContextForTest(), paramsWithBelowMinimumWeight)
		suite.NoError(err)
		suite.Equal(ddsfscPriceCents, priceCents)
	...
	suite.Run("DDSFSC is negative if fuel price from EIA is below $2.50", func() {
		isPPM := false
		priceCents, _, err := DomesticDestinationFuelSurchargePricer.Price(suite.AppContextForTest(), ddsfscActualPickupDate, ddsfscTestDistance, ddsfscTestWeight, ddsfscWeightDistanceMultiplier, 242400, isPPM)
		suite.NoError(err)
	...
	suite.Run("Price validation errors", func() {
		...
		_, _, err := DomesticDestinationFuelSurchargePricer.Price(suite.AppContextForTest(), time.Time{}, ddsfscTestDistance, ddsfscTestWeight, ddsfscWeightDistanceMultiplier, ddsfscFuelPrice, isPPM)
		suite.Error(err)
		suite.Equal("ActualPickupDate is required", err.Error())

		// No distance
		_, _, err = DomesticDestinationFuelSurchargePricer.Price(suite.AppContextForTest(), ddsfscActualPickupDate, unit.Miles(0), ddsfscTestWeight, ddsfscWeightDistanceMultiplier, ddsfscFuelPrice, isPPM)
		suite.Error(err)
		suite.Equal("Distance must be greater than 0", err.Error())

		// No weight
		_, _, err = DomesticDestinationFuelSurchargePricer.Price(suite.AppContextForTest(), ddsfscActualPickupDate, ddsfscTestDistance, unit.Pound(0), ddsfscWeightDistanceMultiplier, ddsfscFuelPrice, isPPM)
		suite.Error(err)
		suite.Equal(fmt.Sprintf("Weight must be a minimum of %d", minDomesticWeight), err.Error())

		// No weight based distance multiplier
		_, _, err = DomesticDestinationFuelSurchargePricer.Price(suite.AppContextForTest(), ddsfscActualPickupDate, ddsfscTestDistance, ddsfscTestWeight, 0, ddsfscFuelPrice, isPPM)
		suite.Error(err)
		suite.Equal("WeightBasedDistanceMultiplier is required", err.Error())

		// No EIA fuel price
		_, _, err = DomesticDestinationFuelSurchargePricer.Price(suite.AppContextForTest(), ddsfscActualPickupDate, ddsfscTestDistance, ddsfscTestWeight, ddsfscWeightDistanceMultiplier, 0, isPPM)
		suite.Error(err)
	...
	suite.Run("PriceUsingParams validation errors", func() {
		...
		_, _, err := DomesticDestinationFuelSurchargePricer.PriceUsingParams(suite.AppContextForTest(), missingActualPickupDate)
		suite.Error(err)
		...
		_, _, err = DomesticDestinationFuelSurchargePricer.PriceUsingParams(suite.AppContextForTest(), missingWeightBilled)
		suite.Error(err)
		...
		_, _, err = DomesticDestinationFuelSurchargePricer.PriceUsingParams(suite.AppContextForTest(), missingFSCWeightBasedDistanceMultiplier)
		suite.Error(err)
		...
		_, _, err = DomesticDestinationFuelSurchargePricer.PriceUsingParams(suite.AppContextForTest(), missingEIAFuelPrice)
		suite.Error(err)
