...
func (suite *GHCRateEngineServiceSuite) TestDomesticOriginSITPickupPricerSameZip3s() {
	...
	suite.Run("success without PaymentServiceItemParams", func() {
		...
		appCfg := appconfig.NewAppConfig(suite.DB(), suite.logger)
		priceCents, displayParams, err := pricer.Price(appCfg, testdatagen.DefaultContractCode, dopsitTestRequestedPickupDate, dopsitTestWeight, dopsitTestServiceArea, dopsitTestSchedule, zipOriginal, zipActual, distance)
		suite.NoError(err)
		suite.Equal(expectedPrice, priceCents)
	...
	suite.Run("invalid weight", func() {
		...
		appCfg := appconfig.NewAppConfig(suite.DB(), suite.logger)
		_, _, err := pricer.Price(appCfg, testdatagen.DefaultContractCode, dopsitTestRequestedPickupDate, badWeight, dopsitTestServiceArea, dopsitTestSchedule, zipOriginal, zipActual, distance)
		suite.Error(err)
	...
	suite.Run("bad original zip", func() {
		suite.setupDomesticServiceAreaPrice(models.ReServiceCodeDSH, dopsitTestServiceArea, dopsitTestIsPeakPeriod, dopsitTestDomesticServiceAreaBasePriceCents, dopsitTestContractYearName, dopsitTestEscalationCompounded)
		appCfg := appconfig.NewAppConfig(suite.DB(), suite.logger)
		_, _, err := pricer.Price(appCfg, testdatagen.DefaultContractCode, dopsitTestRequestedPickupDate, dopsitTestWeight, dopsitTestServiceArea, dopsitTestSchedule, "7891", zipActual, distance)
		suite.Error(err)
	...
	suite.Run("bad actual zip", func() {
		suite.setupDomesticServiceAreaPrice(models.ReServiceCodeDSH, dopsitTestServiceArea, dopsitTestIsPeakPeriod, dopsitTestDomesticServiceAreaBasePriceCents, dopsitTestContractYearName, dopsitTestEscalationCompounded)
		appCfg := appconfig.NewAppConfig(suite.DB(), suite.logger)
		_, _, err := pricer.Price(appCfg, testdatagen.DefaultContractCode, dopsitTestRequestedPickupDate, dopsitTestWeight, dopsitTestServiceArea, dopsitTestSchedule, zipOriginal, "12", distance)
		suite.Error(err)
	...
	suite.Run("error from shorthaul pricer", func() {
		suite.setupDomesticServiceAreaPrice(models.ReServiceCodeDSH, dopsitTestServiceArea, dopsitTestIsPeakPeriod, dopsitTestDomesticServiceAreaBasePriceCents, dopsitTestContractYearName, dopsitTestEscalationCompounded)
		appCfg := appconfig.NewAppConfig(suite.DB(), suite.logger)
		_, _, err := pricer.Price(appCfg, "BOGUS", dopsitTestRequestedPickupDate, dopsitTestWeight, dopsitTestServiceArea, dopsitTestSchedule, zipOriginal, zipActual, distance)
		suite.Error(err)
...
func (suite *GHCRateEngineServiceSuite) TestDomesticOriginSITPickupPricer50PlusMilesDiffZip3s() {
	...
	suite.Run("success without PaymentServiceItemParams", func() {
		...
		appCfg := appconfig.NewAppConfig(suite.DB(), suite.logger)
		priceCents, _, err := pricer.Price(appCfg, testdatagen.DefaultContractCode, dopsitTestRequestedPickupDate, dopsitTestWeight, dopsitTestServiceArea, dopsitTestSchedule, zipOriginal, zipActual, distance)
		suite.NoError(err)
		suite.Equal(expectedPrice, priceCents)
	})
...
func (suite *GHCRateEngineServiceSuite) TestDomesticOriginSITPickupPricer50MilesOrLessDiffZip3s() {
	...
	suite.Run("not finding a contract year record", func() {
		...
		appCfg := appconfig.NewAppConfig(suite.DB(), suite.logger)
		_, _, err := pricer.Price(appCfg, testdatagen.DefaultContractCode, twoYearsLaterPickupDate, dopsitTestWeight, dopsitTestServiceArea, dopsitTestSchedule, zipOriginal, zipActual, distance)
		suite.Error(err)
