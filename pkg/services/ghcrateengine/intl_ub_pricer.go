...
const isUBPricerMinimumWeight = unit.Pound(300)

type intlUBPricer struct {
}

func NewIntlUBPricer() services.IntlUBPricer {
	return &intlUBPricer{}
}

func (p intlUBPricer) Price(appCtx appcontext.AppContext, contractCode string, referenceDate time.Time, weight unit.Pound, perUnitCents int) (unit.Cents, services.PricingDisplayParams, error) {
	...
	if weight < isUBPricerMinimumWeight {
		return 0, nil, fmt.Errorf("Weight must be at least %d pounds", isUBPricerMinimumWeight)
	}
	if perUnitCents == 0 {
...
func (p intlUBPricer) PriceUsingParams(appCtx appcontext.AppContext, params models.PaymentServiceItemParams) (unit.Cents, services.PricingDisplayParams, error) {
