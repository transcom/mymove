...
func (p domesticDestinationSITDeliveryPricer) Price(appCtx appcontext.AppContext, contractCode string, referenceDate time.Time, weight unit.Pound, serviceArea string, sitSchedule int, zipDest string, zipSITDest string, distance unit.Miles, disableMinimumWeight bool) (unit.Cents, services.PricingDisplayParams, error) {
	return priceDomesticPickupDeliverySIT(appCtx, models.ReServiceCodeDDDSIT, contractCode, referenceDate, weight, serviceArea, sitSchedule, zipDest, zipSITDest, distance, disableMinimumWeight)
}
...
func (p domesticDestinationSITDeliveryPricer) PriceUsingParams(appCtx appcontext.AppContext, params models.PaymentServiceItemParams) (unit.Cents, services.PricingDisplayParams, error) {
	...
	serviceAreaDest, err := getParamString(params, models.ServiceItemParamNameSITServiceAreaDest)
	if err != nil {
		return unit.Cents(0), nil, err
	}
