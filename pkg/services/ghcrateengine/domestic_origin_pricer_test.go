...
func (suite *GHCRateEngineServiceSuite) TestPriceDomesticOriginWithServiceItemParamsBadData() {
	...
	suite.Run("failure during pricing bubbles up", func() {
		suite.T().Skip("this test will only pass if disableMinWeight is false on the pricer")
...
func (suite *GHCRateEngineServiceSuite) TestPriceDomesticOriginWithServiceItemParams() {
	...
	suite.Run("successful pricing for hhg with weight < 500 lbs", func() {
		...
		weightBilledIndex := 3
		displayParams := suite.conductHHGMinWeightTests(models.ReServiceCodeDOP, weightBilledIndex, params, paymentServiceItem)
...
func (suite *GHCRateEngineServiceSuite) TestPriceDomesticOrigin() {
	...
	suite.Run("weight below 0", func() {
		...
		suite.Equal(unit.Cents(0), cost)
		suite.Error(err)
		suite.Equal("weight -250 is not a valid number", err.Error())
	})
