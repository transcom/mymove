...
// FetchUploadInformationForDeletion fetches upload information for an uploading being deleted
// difference is this could return multiple values since a serviceMember can have multiple moves
// adding in the moveLocator as a filter param
func (uif *uploadInformationFetcher) FetchUploadInformationForDeletion(appCtx appcontext.AppContext, uploadID uuid.UUID, moveLocator string) (services.UploadInformation, error) {
	q := `
SELECT uploads.id as upload_id,
       uploads.content_type,
       uploads.created_at,
...
where uploads.id = $1
    AND moves.locator = $2`
	ui := services.UploadInformation{}
	err := appCtx.DB().RawQuery(q, uploadID, moveLocator).First(&ui)
	if err != nil {
		switch err {
		case sql.ErrNoRows:
			return services.UploadInformation{}, apperror.NewNotFoundError(uploadID, "")
