...
type DomesticLinehaulPricer interface {
	Price(contractCode string, requestedPickupDate time.Time, isPeakPeriod bool, distance unit.Miles, weight unit.Pound, serviceArea string) (unit.Cents, []PricingParam, error)
	ParamsPricer
}
...
type DomesticOriginFirstDaySITPricer interface {
	Price(contractCode string, requestedPickupDate time.Time, isPeakPeriod bool, weight unit.Pound, serviceArea string) (unit.Cents, []PricingParam, error)
	ParamsPricer
}
