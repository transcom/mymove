...
.PHONY: deps
deps: prereqs check_hosts check_go_version check_node_version check_gopath check_bash_version ensure_pre_commit client_deps server_deps ## Run all checks and install all depdendencies
...
.PHONY: client_build
client_build: check_node_version .client_deps.stamp .client_build.stamp ## Build the client
