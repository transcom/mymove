...
6. Test the secure migration by running `scripts/run-prod-migrations` to setup a local  prod_migrations` database. Then run `env DB_NAME=prod_migrations scripts/psql < tmp/$NAME_OF_YOUR_SECURE_MIGRATION`. Verify that the updated values are in the database.
