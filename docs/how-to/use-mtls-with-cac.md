...
```bash
ENVIRONMENTS=experimental ./scripts/upload-secure-migration tmp/20200211150405_mr337_cac.up.sql
ENVIRONMENTS=staging ./scripts/upload-secure-migration tmp/20200211150405_mr337_cac.up.sql
```
...
One completed one can confirm the upload with the `compare-secure-migrations` tool. The `prod` migration should be much smaller since just a stub file.

```
Migration: secure-migrations/20200211150405_mr337_cac.up.sql
	 experimental 	 1421
	 staging 	 1421
	 prod 	 150
```
