...
class TioFlowPage {
   ...
   * @param {object} move
   */
  constructor(page, officePage, move) {
    this.page = page;
    this.officePage = officePage;
    this.move = move;
    this.moveCode = move.locator;
...
test.describe('TIO user', () => {
  ...
  test.describe('with HHG Moves', () => {
    test.beforeEach(async ({ page, officePage }) => {
      const move = await officePage.buildHHGMoveWithServiceItemsandPaymentRequestsForTIO();
      await officePage.signInAsNewTIOUser();

      tioFlowPage = new TioFlowPage(page, officePage, move);
      await officePage.tioNavigateToMove(move.locator);
    });

    test('can use a payment request page to update orders and review a payment request', async ({
      page,
      officePage,
    }) => {
      ...
      expect(page.url()).toContain('/payment-requests');
      await expect(page.getByTestId('MovePaymentRequests')).toBeVisible();
      const reqCount = await page.locator('[data-testid="MovePaymentRequests"] [data-testid="tag"]').count();
     ...
     * the initial playwright conversion
     */
  ...
  test.describe('with NTSR moves without service items', () => {
    ...
    test('can reject a NTS-R', async ({ page, officePage }) => {
      // Payment Requests page
      expect(page.url()).toContain('/payment-requests');
      await expect(page.getByTestId('MovePaymentRequests')).toBeVisible();
  ...
  test.describe('with NTSR moves with service items and payment request', () => {
    test.beforeEach(async ({ page, officePage }) => {
      const move = await officePage.buildNTSRMoveWithServiceItemsAndPaymentRequest();
      await officePage.signInAsNewTIOUser();

      tioFlowPage = new TioFlowPage(page, officePage, move);
      await officePage.tioNavigateToMove(move.locator);
    });
    test('can view calculation factors', async ({ page, officePage }) => {
      // Payment Requests page
      expect(page.url()).toContain('/payment-requests');
      await expect(page.getByTestId('MovePaymentRequests')).toBeVisible();
